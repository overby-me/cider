...
1. **#79 IS FIXED. The cascade is cut, 1,558 builders to 44.**
   ...
   **ONLY THE FIRST ROW IS A LEAK, and calling the second one a second cascade was wrong.**
   ...
   darlingserver GROUP STORE interpolated as `_g`, with no dependency edge behind it, so
   `root//src/libsimple:libsimple_darlingserver` recompiles `src/lock.c` for a file it does not
   use. That is #79. For `rtsig.c` the cause is a real edge: `src/startup/BUCK` declares
   `host_gen(name = "rtsig_header", out = "rtsig.h", tool = ":rtsig")`, so the header is
   GENERATED BY RUNNING the binary built from `rtsig.c`. Editing it legitimately rebuilds the
   tool, regenerates `rtsig.h`, recompiles the **14** libsystem objects that include it, and
