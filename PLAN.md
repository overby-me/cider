...
`SRC_STORE_RE` in `gen-buck-from-ninja (deleted)`, `PROJECT_MARKERS` in `buck-host-includes.nu` (which
made 98.7 percent of its population noise and kept it red for days), and `SRC_STORE_RE` again
in `mig-from-ninja (deleted)` (all 124 mig edges). When a check is permanently red and someone
...
path. For classes 3 and 4 the generator is the arbiter: run `gen-buck-from-ninja (deleted) <target>`
...
And the reference build is a wasting asset: mig-from-ninja (deleted), gen-buck-from-ninja (deleted)
...
`gen-buck-from-ninja (deleted)`, and nearly all are `# GENERATED by scripts/X` headers in the files it
