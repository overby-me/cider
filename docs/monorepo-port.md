...
|---|---|---|---|---|
| `gen-buck-from-ninja (deleted)` | 2,508 | `result-graph-ref/build.ninja` | imported as a LIBRARY by 6 scripts | **split**: 182 live, 2,326 archive |
...
A plain grep for a script name reports `gen-buck-from-ninja (deleted)` in 2,899 places and
...
`mig-from-ninja (deleted)` and `gen-xtrace-mig.py` drop to zero and the two `--repo-roots` generators
stay live.

### The big one is 7 percent live, not 100 percent dead

`gen-buck-from-ninja (deleted)` is not a standalone tool. Six scripts load it as a module, and four of
...
| --- | --- | --- |
| `gen-buck-from-ninja (deleted)` | 2 | `gen-install-from-manifests` (generator) and `buck-port` (tool). `gen-xtrace-mig`, `buck-fix-link-model` and `regen-dylibs` are gone: the first is nushell, the other two are Rust, and the 182-line live surface they shared now lives ONCE in `graph-specs/src/ninjaref.rs` |
