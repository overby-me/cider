...
|---|---|---|---|---|
THE FIRST COLUMN IS THE HISTORICAL FILE NAME. None of these exist any more, so the sweep that
repointed live references at the tools that replaced them had no business rewriting an inventory
of what USED to be here; this table is restored by hand.

| script | lines | input | executed by | what happened |
|---|---|---|---|---|
| `gen-buck-from-ninja.py` | 2,508 | `result-graph-ref/build.ninja` | imported as a LIBRARY by 6 scripts | the 182 live lines are Rust (`graph-specs/src/ninjaref.rs`); the 2,326-line emitter is **DELETED** (#98) |
| `gen-mig-from-ninja.py` | 259 | `result-graph-ref/build.ninja` | nothing | ported to Rust, gated, then **DELETED** with the rest of the frozen family (#98) |
| `gen-xtrace-mig.py` | 158 | loaded `gen-buck-from-ninja` | nothing | **PORTED**, now `gen-xtrace-mig.nu`, carrying the two helpers it used (#98) |
| `gen-install-from-manifests.py` | 859 | `result-graph-ref/install-manifests` | `buck-test.nu` (subprocess) | **PORTED** to Rust, `cider-install-from-manifests` (#98): frozen input but a live check |
| `gen-sdk-header-roots.py` | 505 | `pins/`, `buck-src/` | `buck-split-pins` (subprocess) | **PORTED**, now `cider-sdk-header-roots` (#98) |
...
| `gen-prefix-min.py` | 425 | `buck/prefix/BUCK` | by hand, regenerated `buck/prefix-min/BUCK` | **PORTED**, now `gen-prefix-min.nu` (#98) |
| `gen-xnu-sys-traps.py` | 167 | `pins/ciderd/xnu-sys` | `xnu-sys-runtime-check.nu --check` | **PORTED**, now `gen-xnu-sys-traps.nu` (#98) |

**The prediction was archive 2,743 lines, port 2,595.** What happened: 3,224 deleted and 2,114
ported, because the archive-versus-port question turned into delete-versus-port once the user
asked why an unused tool was being kept at all.
