...
**Pass 114 (task #11: persistent-container reuse IMPLEMENTED, verified, committed -- second cheap launcher win).** Implemented the Pass 113 design in src/linux/launcher/src/main.rs: before the rootless unshare gate, try_join_live_container(prefix) validates a live container (.init.pid + kill(pid,0) + comm==ciderd + shellspawn socket present) and, from the parent userns, setns(CLONE_NEWUSER) into the child container's userns, returning its pid; main() then skips the unshare gate and the get/reap/create block and REUSES that pid. Only the userns is joined early; the mount-ns join stays at its original point so prefix bootstrap and cwd are still read in the host namespace (behavior-preserving). Falls back to the create path when no live container exists. Built via nix build .#launcher (seconds).

Verified on the min prefix (installed next to ciderd as rt/bin/cider.persist so ds_bin_path resolves):
- Correctness: exec true rc=0; exec bash and shell bash both compute 42.
- Persistence: ciderd pid is now STABLE across spawns (was: changed every spawn). 50/50 sequential spawns -> 42; ciderd fd count flat 9->9 and init-mldr 10->10 across 50 spawns (the shellspawn shellfd[0]-closed-3x nit is benign here, no EMFILE risk); 8/8 concurrent parallel joins to the one container all correct.
- Wall-time, direct exec (cider exec true), median of 15: 100ms-poll baseline 154ms -> 1ms poll 64ms -> +reuse 40ms. Full arc 154->40ms = 74% faster (3.85x). Shell: 143->128ms (login chain dominates there). Committed vklknxvl "perf(cider): reuse the persistent container across rootless spawns".
- Safety: no privilege escalation -- joining the child userns maps to the same host uid as before; isolation identical to the create path.

#11 SCOREBOARD (direct-exec guest spawn hot path, the build-relevant metric): 154ms -> 40ms via two cheap LAUNCHER-only changes (1ms socket poll + persistent-container reuse), no guest prefix rebuild. The dead-end levers (RPC reduction, dyld shared cache, image-load) were all retired on wall-time; the real cost was launcher-side spawn overhead. Both fixes ship automatically in the next full prefix build (source committed). Remaining headroom is small and mostly guest-side (~30ms residual = mldr+dyld+program+exit; login chain adds ~85ms but only affects interactive `cider shell`, not builds). Recommend: bank these wins; the next-biggest lever would need a full-prefix build+profile of a REAL nix workload (blocked on #24 JSC for the full prefix) or guest-side exec work -- both bigger and lower-confidence than what was just banked. Good point to hold for direction.

**Pass 115 (task #11: decomposed the 40ms residual -- confirms the cheap launcher lever is exhausted; holding).** With cider.persist (1ms poll + container reuse), warm, median of 15, fast-exit decomposition of the direct-exec spawn:
- attach-only (exec /nonexistent, dies before the guest): ~14ms = host launcher side (setns join into the container userns + prefix bootstrap/ensure_prefix_dirs + mnt-ns join + shellspawn socket connect/handshake). Near-irreducible.
- full (exec true): ~40-46ms. Delta ~= ~30ms = GUEST exec (mldr loads the Mach-O + dyld + trivial program + Mach/exit teardown). This is the same ~37ms image-load-ish cost measured in Pass 111; Lever A (dyld cache) already proven a net regression against it.

Conclusion: after the two launcher wins (154->40ms), the remaining ~14ms is lean host-side work and the ~30ms is guest-side and needs a prefix rebuild + guest work to touch (low confidence: image-load is not compressible via the shared cache, so it would mean mldr/dyld startup surgery). Nothing CHEAP remains on the per-spawn path. The launcher lever is exhausted. Bigger remaining options are all costlier/blocked: guest-side mldr/dyld work (2h rebuild), login-chain ~85ms (interactive shell only), or an end-to-end real-build measurement (blocked on #24 full prefix). Holding for direction rather than spending a speculative 2h rebuild.

**Pass 116 (task #11/#28: fixed a per-login wasted guest spawn on the `cider shell` login chain -- third cheap launcher win + a correctness bug).** Investigated the ~85ms login-chain overhead flagged in Passes 111-114. The guest /etc/profile copies `/System/Library/User Template/Library` into `~` guarded by `[ ! -d ~/Library ]`, so it is idempotent BY DESIGN. But a login probe (run twice against one warm container) showed the guard never trips: HOME=/Users/root (setup_shellspawn_env, main.rs:773) yet the image ships no /Users at all (only /var/root exists), so `cp -r ... ~/` fails ENOENT every login and, never creating ~/Library, re-runs the doomed copy on EVERY subsequent login. Two bugs in one: a wasted guest process spawn per login, and ~/Library never actually set up. (Also noted an internal inconsistency: the launcher-generated passwd at main.rs:376/384 says root home=/var/root, while HOME and src/darwin/etc/master.passwd say /Users/root; not touched, the fix targets the observed $HOME.)

FIX (launcher only, cheap build via nix build .#launcher, no guest rebuild): ensure_prefix_dirs now creates /Users and the home dir (/Users/root + /Users/{name}). It already runs on EVERY launch and self-repairs old prefixes (main.rs:122), so existing prefixes get the dir too. Verified with the login probe on cider.home: login 1 cp now SUCCEEDS (/Users/root + /Users/root/Library created and persisted on disk), login 2 SKIPS the cp (guard trips), zero errors (the whole ENOENT spam gone). Wall-time A/B, warm persistent container, N=10 `cider shell /usr/bin/true`: BEFORE (cp fails every login) median 135ms -> AFTER (cp once) median 93ms = ~42ms/login, ~31% faster; plus ~/Library is now correctly populated. Committed. Ships in the next full prefix build (source committed). Remaining per-login cost: path_helper still forks unconditionally each login (guest-side, small). This is the `cider shell` interactive path; the build hot path (direct exec, 40ms) does not touch the login chain.

**Pass 117 (task #11/#29: END-TO-END aggregate validated with NO heavy build; JSC/#24 is a FALSE blocker for perf).** Reframed #29: the full prefix (blocked on #24, JSC arm64 LLInt/WASM offline-asm link) is NOT needed for the perf goal. This file already records (lines ~1422, ~1698) that bash and nix do not need JSC, and the min prefix builds. So end-to-end validation needs no ~2h/full build at all. Measured the build-relevant aggregate directly on the existing min prefix (rt/) with the committed launcher binaries: N=60 sequential `cider exec <guest true>` (one per build action, as buck2 drives), warm, 60/60 correct:
- BASELINE cider.orig (100ms poll + container REBUILT every spawn): total 9.09s, 151ms/invocation.
- FIXED cider.home (1ms poll + persistent container + login home fix): total 2.49s, 42ms/invocation.

= 3.65x faster aggregate (6.6s saved over 60 actions), scaling linearly with action count (a 10k-action build sheds ~25min -> ~7min of cider spawn overhead). The three launcher wins are now validated END-TO-END without any guest rebuild. The #11 cheap-launcher arc is COMPLETE and validated on wall-time. Remaining headroom is the ~30ms guest-side exec residual (mldr/dyld + program + exit), which needs guest surgery + a prefix rebuild and is low-confidence (Lever A already proved image-load is not compressible via the shared cache); deferred pending direction, not auto-started.

**Pass 118 (task #11/#22: decomposed the ~30ms guest floor -- dyld is only ~5ms; the sole remaining lever is the ~21ms Mach exec-setup + teardown RPC).** Honest real-world aggregate first (N=40 cider exec, cider.orig vs cider.home): true 152->40ms = 3.82x, ls 150->43ms = 3.47x. `ls` (real dylibs + getdents/stat) adds only ~3ms over `true`, so the guest side is a roughly FIXED ~30ms floor per action, not proportional to the program: the ~3.5x holds for real actions, it is not a `true`-only artifact. DYLD_PRINT_STATISTICS (the guest dyld honors host env; NO rebuild) on both: total pre-main ~5.1-5.4ms (dylib loading ~1.5ms, rebase/binding ~0.7ms, ObjC ~0.3ms, initializers ~2.6ms of which libSystem ~1.9ms). So dyld is only ~5ms of the floor, which INDEPENDENTLY CONFIRMS Lever A's retirement (the shared cache could save at most the ~2.2ms of load+bind, far below its ~30ms map cost). The 40ms warm spawn now decomposes as: ~14ms host launcher (setns join + prefix bootstrap + shellspawn socket handshake; near-irreducible) + ~5ms dyld (not worth touching) + ~21ms Mach exec-setup + teardown RPC (guest shellspawn/mldr <-> ciderd task/thread/port create, then exit teardown). That ~21ms Mach RPC path is the ONLY remaining perf lever = tasks #22 (in-guest Mach/signal fast paths) / #23 (no-ciderd). Next cheap step before any guest rebuild: split the ~21ms into its ciderd-side (Linux daemon, fast-build like the launcher) vs guest-side mldr/shellspawn (needs the ~2h prefix rebuild) portions, and try any ciderd-side win first.

**Pass 119 (task #22: scoped the ~21ms Mach exec/teardown lever to the bottom -- it is RPC round-trip COUNT, not ciderd CPU; the win is the unimplemented exec-replacement refinement, and ciderd is NOT cheap-buildable).** Read the ciderd (src/linux/server, Rust) exec+exit handlers: set_executable_path (handler.rs:973) just stores the path; checkin (:418) ups the parent fork semaphore; checkout (:437) marks the guest thread dying and reaps. Each handler is microseconds of CPU, so the ~21ms/spawn is NOT ciderd compute: it is the round-trip count x latency of the guest<->ciderd Mach RPC sequence for spawn+exec+exit (guest fork/clone + mldr map + task/thread/port setup RPCs + exit-teardown RPCs), on top of the ~5ms guest dyld. Two handler comments name the known win: handler.rs:417 (\"Exec-replacement's task/thread swap is a later refinement\") and :436 (\"The exec-listener branch is a later refinement\"). Today `cider exec` runs a full fork -> checkin -> exec -> teardown cycle; an exec-REPLACEMENT that swaps task/thread in place would cut round-trips. That is a substantial ciderd feature, and ciderd has NO fast standalone build (nix/launcher.nix builds only the launcher; ciderd builds via buck2 //src/linux/server:ciderd, the heavy prefix path). So #22 is real ciderd dev plus bounded buck2 build-test cycles (memory-careful, one at a time, never a whole-tree eval), NOT a cheap launcher-style win. Decision point: invest in the exec-replacement refinement, or treat the validated ~3.5x launcher arc as the perf result. Design work can proceed cheaply from code reading before any build.

**Pass 120 (task #22: MEASURED the ~21ms directly -- it is ~55 guest<->ciderd RPC round-trips per warm spawn, dominated by in-guest Mach port chatter).** DSERVER_TRACE_CALLS makes ciderd log every RPC to {prefix}/ciderd.log (launcher redirects ciderd stdio there, main.rs:576); no build needed. ONE warm `cider exec true` spawn = ~55 RPC round-trips (idle-noise baseline 0), each its own epoll wakeup with NO batching -- at a few tenths of a ms apiece that is the ~21ms. Round-trips per spawn by call: MACH_MSG_OVERWRITE #38 x10, MACH_REPLY_PORT #36 x8, MACH_PORT_DEALLOCATE #39 x5, TASK/HOST/THREAD_SELF_TRAP #33/#34/#35 x4 each, UIDGID #7 x4, SET_THREAD_HANDLES #8 x3, VCHROOT_PATH #3 x3, CHECKIN #1 x3, FORK_WAIT_FOR_CHILD #11 x2, CHECKOUT #2 x1, KQCHAN_PROC_OPEN #29 x1, MLDR_PATH #10 x1, STARTED_SUSPENDED #5, GET_TRACER #6. So the ~21ms is guest process startup making many SYNCHRONOUS Mach traps (self-port lookups + reply-port alloc/dealloc + mach_msg to bootstrap) each proxied to ciderd, plus the fork->checkin->exec->checkout lifecycle (two nsids: the forked shellspawn child and the exec'd image). That is exactly task #22's two named levers: (a) in-guest FAST PATHS for the constant Mach ports (cache mach_task_self/host_self/thread_self; pool reply ports) so they never round-trip; (b) exec-replacement task/thread swap to drop the transient fork's setup. BOTH live guest-side (libsystem_kernel / mldr) or cross-component (ciderd), so BOTH need heavy builds (the ~2h prefix or a guest libsystem rebuild), NOT a cheap launcher win. Realistic upside is ~5-10ms off the 40ms (~1.15-1.33x), diminishing returns against the banked ~3.5x. RECOMMENDATION: bank the ~3.5x launcher arc as the perf result; pursue #22 only on an explicit go, since it is a multi-build guest-side campaign under the don't-waste-time / don't-OOM guardrails.

**Pass 121 (task #22/#23: user set the real end-goal = an xnu-rpc-free MODE; wrote docs/xnu-rpc-free.md design; found self-port caching is already done and the transient fork re-init is the wasted work).** The user (2026-08-27) named an xnu-rpc-free mode (guest runs without ciderd round-trips) as an end-goal alongside perf, which is the SAME target as the ~55 round-trips/spawn. Read the guest Mach-trap layer to ground a design (all cheap, no build):
- Guest Mach traps are in vendor/src/xnu/darling/src/libsystem_kernel/.../mach/impl/mach_traps.c; each `*_impl` calls a `dserver_rpc_*` wrapper that round-trips to ciderd. That is the cost.
- IN-GUEST PRECEDENT already exists: self-targeted VM ops short-circuit to Linux syscalls (mach_vm_allocate/map->mmap ~L235/339, deallocate->munmap ~L273, protect->mprotect ~L303) instead of RPC. The xnu-rpc-free mode extends this pattern from self-VM to self-ports/self-IPC.
- Self-ports are ALREADY cached: libsyscall mach_init_doit (vendor/src/xnu/libsyscall/mach/mach_init.c:192-193) sets mach_task_self_ = task_self_trap() and _task_reply_port = mach_reply_port() ONCE per init, reset on fork via _mach_fork_child. So "cache self-ports in-guest" is a DEAD END; the self-trap count comes from multiple per-process/per-image inits, not repeated calls.
- The wasted work: shellspawn spawns via fork() (shellspawn.c:281) + execv() (:290). The public fork() runs the FULL atfork child path (_mach_fork_child -> mach_init_doit -> task_self_trap + reply_port + mach_driver_init) which execv then discards. posix_spawn (posix_spawn.c) uses a leaner posix_spawn_child path and skips user atfork. shellspawn is a bounded buck2 target (//src/darwin/shellspawn:shellspawn, one C file), so this is the most tractable first step and is testable without the full prefix.

Wrote docs/xnu-rpc-free.md: how guest Mach ops reach ciderd, the in-guest precedent, the Pass 120 round-trip table, a categorization (A redundant re-init / B in-guest-able self-IPC / C genuinely-needs-ciderd), a prioritized incremental plan (1 shellspawn fork->posix_spawn; 2 fold uidgid/vchroot/mldr_path/set_thread_handles into checkin; 3 exec-replacement task/thread swap; 4 guest-local Mach port namespace), correctness hazards, and how to measure (rpc-count.sh + agg-ab.sh). Deliberately did NOT run guest builds overnight (unsupervised OOM risk per the don't-OOM guardrail); the design is ready to execute with bounded builds when supervised.

**Pass 122 (task #11/#23: CRITICAL reconciliation -- the overnight fat-checkin design REDISCOVERED task #25, which was already TRIED and REVERTED as a wall-time non-win; RPC count is a proven false proxy here).** While preparing to implement the fat checkin (fold self-ports + setup RPCs into the checkin reply), found task #25 was EXACTLY this checkin-reply fold: implemented Pass 99 (bd2d2384; checkin reply {task_self,uid,gid}; boot RPC 236->224), extended Pass 100 (v2 vchroot, ->219), then REVERTED Pass 102 because Pass 101's wall-time A/B measured NO boot benefit and ~2.5ms/spawn SLOWER on spawn-heavy workloads (the eager server-side reply-fill cost more than the round-trips saved), despite -17 boot RPC. The passes' repeated lesson: RPC count is a FALSE PROXY for wall-time; gate perf on wall-time, never RPC/syscall counts. The current tree confirms the revert: the checkin reply is header-only again and mach_traps.c has no task_self leaf-cache (getuid.c's stored_uid/gid cache survives; the mldr/lkm.c apple[] seeding is gone). CONSEQUENCE: the fat checkin (and the xnu-rpc-free RPC-reduction thesis for PERF) is likely a wall-time non-win or slight regression per strong prior evidence; the overnight ~1.5x estimate was RPC-count-based and is unreliable. Added a PERF CAVEAT to docs/xnu-rpc-free.md: the doc stands as the design for the xnu-rpc-free MODE (architecture / reduced daemon coupling), but its PERF payoff is unproven and must be wall-time-prototyped before any build. This also reconciles with the session's warm measurement: the ~21ms "Mach RPC" was attributed by SUBTRACTION (40 - ~14 host - ~5 dyld), NOT proven to be round-trip latency on the critical path; prior work indicates image-load/mldr dominates. Next: do NOT build a proven-class non-win; instead profile the warm ~21ms DIRECTLY (wall-time) to find the true bottleneck, and only pursue the fold if a wall-time prototype shows round-trips are actually on the critical path in the new persistent-container warm regime.

**Pass 123 (task #11/#22/#25: WARM-regime wall-time A/B of the resurrected #25 checkin fold -- NO reliable win, within noise; RPC-count reduction confirmed a false proxy EVEN in the warm regime).** Re-opened the #25 question (Pass 122) because the prior revert (Pass 101/102) measured a BOOT-regime regression, while this session's persistent-container fix created a NEW warm regime (dyld only ~5ms). Resurrected #25 v1+v2 (task_self/uid/gid/vchroot folded into the checkin reply) from history: `jj restore --from abecd0d4` of the touched files (handler.rs, rpc_wire.rs, generate-rpc-wrappers.py, loader main/rpc/stack.rs, xnu patches 0045/0046) onto the current no-#25 base; wire regenerated + validated byte-identical; correctness gate PASSED (bash 42, uid 0:0, rc=0, no hang). Built both min prefixes under a memory watchdog: #25 = result-xnu25 (fast, nix-store cache hit); clean same-base no-#25 baseline = result-baseline (a ~5.5h FULL-CLOSURE rebuild, novel source hash so no cache).

WARM A/B, persistent container, warm `cider exec true` MEDIAN ms (#25 vs baseline):
- PRELIMINARY (confounded, izrd7995 baseline): BASELINE 48.6 / XNU25 44.2 -> looked ~4ms faster.
- CLEAN run 1 (N=20): BASELINE 52.7 / XNU25 46.6 -> #25 -6.1ms (faster).
- CLEAN run 2 (N=20): BASELINE 46.2 / XNU25 50.1 -> #25 +3.9ms (slower).
- CLEAN run 3 (N=40): BASELINE 39.9 / XNU25 45.5 -> #25 +5.6ms (slower).
The SIGN FLIPS across runs and the run-to-run median swing (~40-53ms, ~13ms) DWARFS any #25 effect (~4-6ms). Mins are equally inconsistent (#25 faster in 2/3, slower in 1/3).

VERDICT: #25 has NO reliable effect on warm wall-time -- it is WITHIN MEASUREMENT NOISE. The promising preliminary/first-run was noise; the 3-run confirmation caught it. This CONFIRMS the standing lesson (RPC count is a FALSE PROXY for wall-time) even in the warm persistent-container regime. #25 stays PARKED on bookmark xnu25-warm-ab (NOT merged). The perf ceiling remains the launcher arc (~3.5x, banked). The warm ~21ms is NOT reducible by cutting the ~55 Mach round-trips (they are not the serial critical path); it is guest-side image-load/mldr, the separately-blocked dyld problem.
...
**Pass 125 (task #11: FULL warm-spawn split via launcher+shellspawn CIDER_TIMING -- transient fork atfork-reinit is ~5ms of the ~14ms; AND discovered the fast Buck2 dev loop, a big workflow fix).** Added shellspawn CIDER_TIMING marks (pre-fork/child-post-fork/child-pre-execv). Warm `cider exec true`, launcher [sent->started] ~12-14ms decomposes: [pre-fork -> child-post-fork] = fork() + atfork Mach re-init = ~5-6ms (the transient init execv discards); [child-post-fork -> child-pre-execv] ~0.05ms (close/fcntl); the remaining ~6-8ms is shellspawn per-spawn setup (setsid/dup2/pipe) + the execv(mldr) emulation. [started->exit] ~10ms = guest run (mldr init + dyld ~5ms + program + teardown). FULL ~24ms warm spawn = ~1.5ms host launcher + ~5ms transient fork atfork-reinit (WASTED) + ~7ms shellspawn-setup+execv-emulation + ~10ms guest run. Levers by size: guest run ~10ms (mostly inherent; dyld is the retired shared-cache problem), execv-emulation ~7ms, transient re-init ~5ms (cleanest waste: skip atfork for the immediate-exec child).

WORKFLOW DISCOVERY (prompted by the user asking why builds are slow / could rebuilds be reduced): iterating guest code via `nix build .#cider-buck2-prefix-min` rebuilds the WHOLE closure (~1-5h) on ANY edit, because with sourceGroups off every target shares one whole-tree source input. The project ships a BUCK2 dev loop (README: "Nix is the packaging, not the build"): `nix develop` + `scripts/buck-setup.nu` (writes .buckconfig.local) + `buck2 build //target`, which is incremental (~97s cold, ~35s warm; fs_hash_crawler tree-sync dominates since watchman won't start below nice 0). Switched to it: shellspawn builds in ~35s vs hours. This unblocks ALL guest-side perf iteration (the overnight #25 A/B and this profiling were needlessly slow on the nix path). Recorded in memory cider-buck2-dev-loop. The nix path could ALSO be made incremental by getting sourceGroups (per-target source staging, #54) robust against dangling relative header symlinks -- a real build-system improvement.

**Pass 126 (task #11: vfork to skip the transient fork's ~5ms atfork Mach re-init BREAKS exec -- the fork's checkin/Mach setup is load-bearing; also found shellspawn is a DOUBLE fork; guest-spawn cost is largely architectural).** Using the fast Buck2 loop (edit shellspawn.c -> `buck2 build //src/darwin/shellspawn:shellspawn` ~35s -> drop into a test rt -> measure), tried `fork()`->`vfork()` (vfork runs no atfork handlers) with a vfork-safe child. Result: [sent->started] dropped ~13ms -> ~7.5ms BUT correctness FAILED (exec true rc=1, 0/5 spawns ok, shell produced no output). So the ~5ms is NOT wasted: the fork's checkin (sys_fork registers the child with ciderd) and/or the atfork Mach setup is REQUIRED for the child to `execv` under cider -- vfork skips it, the child cannot complete the exec RPCs, mldr/guest fails. vfork reverted. ALSO: shellspawn is a DOUBLE fork (shellspawn.c:64 comment; :121 forks a handler per connection, :293 forks the target), so each spawn pays TWO fork atfork re-inits (the first sits in the [sent->pre-fork] segment I attributed to setup). CONCLUSION: the guest-side warm-spawn cost is largely ARCHITECTURAL (double-fork + per-process Mach re-init + exec emulation + mldr + dyld ~5ms), not a cheap lever; the banked launcher arc (~3.5x) already captured the cheap host-side gains. Deeper reductions -- collapse the double-fork / handle connections inline, a ciderd exec-replacement that reuses the task/thread instead of teardown+rebuild, or in-guest Mach -- are real architecture work with correctness risk and uncertain payoff, and Passes 124/125 localize exactly where they must act. Lasting win of this arc: the Buck2 dev loop makes guest iteration ~35s not hours (memory cider-buck2-dev-loop), so these experiments are now cheap to run and reject.

**Pass 127 (task #11/#30: the ciderd exec-replacement is ALREADY implemented -- task reuse across execve is in place (task #52); the remaining warm-spawn cost is the DOUBLE FORK + per-process init, which is architectural).** Went to implement the exec-replacement and found it already done. The serve loop (ciderd.rs:355-368) reads the checkout (call #2) body: `exec_listener_pipe >= 0` means an execve (the process lives on) vs `< 0` a real exit; it `prune_process` (drops the ProcState / xnu_sys task) ONLY on a real exit. So across execve the TASK is REUSED, not torn down + rebuilt (the "later refinement" comment at handler.rs:436 refers to the finer thread-swap, not the task). What still tears down per-exec is only the THREAD (`reap_thread` is unconditional), but the guest (mldr) re-inits its Mach state after exec regardless, so keeping the ciderd thread would not stop the guest re-init -- the SAME reason the #25 RPC fold (Pass 123) and the vfork skip (Pass 126) did not move wall-time. The exec-replacement lever is therefore spent. Warm-spawn ~24ms = ~1.5ms host + the DOUBLE FORK (shellspawn forks a handler per connection AND the target, each a fresh-process task creation + guest mach_init, ~5ms apiece) + execv-emulation ~7ms + guest run ~10ms (mldr + dyld ~5ms + program + teardown). This is architectural. The ONLY remaining tractable lever: collapse the double-fork -- have the daemon fork the target directly and track exits via its event loop, dropping the per-connection handler fork (~5ms) while keeping concurrency. That is a shellspawn concurrency refactor: risky, ~5ms (~20%) payoff, and the fork's checkin is load-bearing (vfork proved). #30 closed as already-implemented.

**Pass 128 (task #11/#23: LANDED the double-fork collapse Pass 127 identified as the last tractable lever -- first GUEST-SIDE warm-spawn win, ~6ms/~13% by wall-time, committed).** Rewrote shellspawn to run ONE central kqueue event loop in the daemon and fork ONLY the target (no per-connection handler fork). The kqueue multiplexes the server socket (accept -> start_spawn), each live spawn's control fd (EVFILT_READ -> signal forwarding), and each target's exit (EVFILT_PROC NOTE_EXIT -> finish_spawn). Every per-process step the handler used to do in its own process (env via deferred putenv, uid/gid, cwd, session, stdio dup2, controlling tty) now runs in the target child, so the daemon never blocks on a spawn's exit and concurrency is preserved. Built via the fast Buck2 loop (~35s/iteration).

BUG found + fixed during bring-up (the instructive part): the first build ran the guest correctly (bash printed 42) yet `cider exec true` returned rc=1 and the launcher never logged "started". Root cause: the "started" byte is written AFTER free_cfg(), and free_cfg walked argv from index 2 assuming argv[0]/[1] were string literals -- true for the shell path but NOT for exec mode, where SETEXEC rebuilds argv as [prog, NULL] (argv[0] malloc'd). So free_cfg read argv[2] out of bounds and freed garbage, corrupting the heap / killing the daemon right before it could send "started"; the already-forked child kept running (hence guest output but rc=1). Fix: strdup the argv[0]/[1] defaults so every entry is uniformly malloc'd, free from index 0, and drop the realloc(argv,0). Added a NOTE_EXIT safety net: register EVFILT_PROC separately so an instant-exit target that races ahead surfaces as kevent()==-1 (ESRCH) and is reaped immediately instead of hanging. setupSigchild switched to SIG_DFL with no SA_NOCLDWAIT so exited targets stay reapable via waitpid in finish_spawn.

INTERLEAVED warm A/B (both daemons warm, alternating runs to cancel drift), `cider exec true`, 30 paired samples:
- base (double fork): median 45.9ms (p25 40.8, p75 50.1)
- collapsed:          median 38.3ms (p25 34.7, p75 41.5)
- PAIRED diff (new-base): median -6.0ms, mean -7.9ms -- collapsed faster on every pair.
Daemon-side launcher [sent->started]: median 11.8ms -> 8.9ms, p90 ~26ms -> ~15ms (the tail, where both forks' atfork stacked, collapses hardest).
Correctness: exec true 10/10, exec false -> rc 1 (exit-code propagation), 8-way parallel 10/10, shell math+id+stdout intact. Committed fbe53f71 (Signed-off-by, no AI attribution).

SIGNIFICANCE: this is the FIRST landed warm-spawn win on the GUEST side -- every prior warm win (Pass 27 arc) was launcher/host-side. It confirms Pass 127's ~5ms/~20% prediction with a PAIRED wall-time median (which beats the ~5ms run-to-run noise that sank the #25 fold in Pass 123, because pairing cancels the between-run drift). It also serves the xnu-rpc-free endgoal directly: collapsing the handler fork removes one full guest-process Mach registration with ciderd (~55 RPCs) per spawn. TRADEOFF (documented, accepted for the trusted-local launcher client): the daemon now does the command-read and exec-check reads inline, so a client that connects and then stalls mid-command would block the loop; the launcher always sends its whole command burst immediately after connect, so this is not a concern in practice. NEXT lever toward both perf and xnu-rpc-free: the remaining single target fork still pays a ~5ms user-space atfork Mach re-init that execv discards; vfork broke exec (Pass 126) because the checkin is load-bearing, but posix_spawn's leaner posix_spawn_child path (xnu-rpc-free.md step 1) skips user atfork while still doing the kernel fork checkin -- worth a Buck2-loop prototype + [sent->started] A/B.

**Pass 128b (task #11/#31: hardened the committed collapse -- the persistent single-process daemon does NOT leak and survives the cases a concurrency refactor breaks).** Turning fork-per-connection into one long-lived event loop makes fd/slot leaks and teardown bugs the primary risk, so exercised them on the built collapsed rt (fast, no rebuild): (1) exit-code fidelity shell `exit N` for N in {0,1,3,42,127} -- all propagated exactly; (2) SOAK 200 sequential exec true -- 200/200, and crucially the shellspawn daemon's own fd count held 12->12 and ciderd 9->9 across all 200 (finish_spawn closes the control fd + 3 stdio copies and frees the slot every spawn; no leak), daemon still alive; (3) 32-way concurrent exec true -- 32/32 (slot allocation under load); (4) interactive/login stdin with NO -c (fed via a pipe) -- stdout correct and exit code 7 propagated, so the login-shell path (setsid + TIOCSCTTY + stdin plumbing) still works; (5) signal/teardown -- a FIFO-blocked guest login shell, SIGTERM to the launcher tore it down in ~18ms and left only the daemon mldr (guest process gone, no orphan). The collapse is production-solid, not just happy-path correct. NEXT-LEVER SCOPING (via the lkm.c/mach_init.c read): the surviving target fork's atfork `_mach_fork_child` does mach_driver_init + task_self_trap + mach_reply_port; mach_driver_init's socket-guard is SKIPPED in the fork child (lkm.c:57-59 "in the child after a fork, sys_fork already takes care of this"), CONFIRMING the doc's claim that sys_fork (not _mach_fork_child) owns the load-bearing RPC socket -- so vfork (Pass 126) broke exec because it bypassed sys_fork, NOT because it skipped the port re-cache. The only wasted RPCs in the exec-bound child are thus the 2 port-cache traps (~1-2ms), which is below the ~5ms wall-time noise and gated behind a delicate guest-core-lib change -- not worth it for perf per the standing "RPC count is a false proxy" lesson. The per-spawn micro-levers are now exhausted; the double-fork collapse was the last tractable one (as Pass 127 predicted).

**Pass 129 (task #11/#23: post-collapse RPC profile + the honest perf verdict -- per-spawn perf is at its FLOOR, the collapse was the last lever; the xnu-rpc-free mode is architecture-only from here).** Grounded the xnu-rpc-free design in post-collapse data (DSERVER_TRACE_CALLS, one warm `cider exec true`): 43 answered RPCs/spawn (was ~55 at Pass 120), ONE target guest nsid + the persistent daemon (the handler nsid and its ~12-RPC handshake are gone). Histogram: mach_msg_overwrite 7, mach_reply_port 6, mach_port_deallocate 4, uidgid 4, task_self_trap 3, thread_self_trap 3, host_self_trap 3, vchroot_path 3, checkin 2, set_thread_handles 2, + 6 singletons. Largest remaining Category-B target: reply_port(6)+dealloc(4) ~= 10 RPCs (Step 4), but perf-null.

RECONCILED THE WHOLE #11 ARC (re-read Passes 86-118 to avoid re-walking closed levers -- nearly restarted the dyld cache before catching that it is retired): every image-load and RPC lever is closed on WALL-TIME evidence -- dyld shared cache maps+serves but dyld is only ~5ms so it is a net regression (Pass 109/118); RPC/checkin fold is within noise (Pass 123); vfork breaks exec (checkin load-bearing, Pass 126); exec-replacement already implemented (Pass 127); _mach_fork_child port re-cache is ~2 RPCs sub-noise behind a delicate core-lib change (Pass 128b). Pass 118 named the ~21ms Mach exec-setup+teardown as the ONE remaining lever, and this session's double-fork collapse attacked exactly it and banked ~6ms/~13% (Pass 128). The residual warm spawn ~22ms = ~1.8ms host + ~9ms single target fork/exec-emulation + ~11ms guest run (mldr/dyld image-load, not compressible). VERDICT: per-spawn guest-execution perf is at its honest floor; the tractable levers are spent and the last win is landed. Updated memory cider-xnu-rpc-free-goal to record the retired levers + that the mode is an ARCHITECTURE goal (perf-null), so no future session re-walks these. The remaining xnu-rpc-free work (Category B: guest-local reply-port namespace) is a large, delicate, two-sided (guest core-lib + ciderd), IPC-correctness-critical build with NO perf payoff -- it needs a user go/no-go before spending it, and is not something to attempt unsupervised (a wrong name-space partition corrupts IPC silently).

**Pass 130 (task #23: reply-port lever REFUTED -- the MIG per-thread TSD reply-port cache already WORKS under cider; not the easy win Pass 92 implied).** Chased the reply_port ~6/spawn as a potential guest-only xnu-rpc-free win (hypothesis: the mig_get_reply_port TSD slot-2 cache is broken under cider's zero-page-before-TSD-established scheme, tls.c). Built an instrumented libsystem_kernel.dylib via the Buck2 loop (//vendor/src/xnu:system_kernel_final, ~72s incremental; vendor sources are mode-444, edit + revert, not jj-tracked): mach_reply_port_impl writes "A" per alloc; mig_get_reply_port writes "m"/"h" per miss/hit. One warm `cider exec true`, target stderr: `hhhhAmAAhhh` = 7 HITS / 1 miss / 3 allocs. So the TSD cache WORKS (it is already saving ~7 RPCs/spawn); the residual allocations are (a) the unavoidable first-mig-call miss per thread and (b) 2 DIRECT mach_reply_port() calls that bypass the mig cache (non-MIG code paths). There is NO "fix the broken cache" win. Instrumentation reverted, vendor files restored + re-locked 444, correctness re-confirmed (exec true rc=0 with the instrumented dylib). REFINED MODE SCOPE: the in-guest fast paths the mode wants are already substantially in place and working (self-VM short-circuit, task_self leaf-cache, mig reply-port TSD cache); the residual ~43 RPCs/spawn are largely irreducible (Category C mach_msg service IPC ~7, non-cacheable host/thread_self, first-init allocs) or only foldable via the proven-null #25 checkin fold. The reply-port local-allocator (doc Step 4) remains the one big Category-B lever, but ciderd's reply ports come from its EMULATED XNU kernel (handler.rs:597 mach::mach_reply_port), so a guest-minted "reserved-range" name risks colliding with XNU index/generation bits and on-demand injection fights the emulated ipc_space -- harder + riskier than the doc assumed. Net: the mode is closer to done than the RPC count suggests, and the remaining reductions are architectural, not quick.

**Pass 131 (task #25/#23: RE-LANDED the checkin fold for the xnu-rpc-free MODE, per user direction -- validated correct + cuts round-trips 43->40/spawn; perf neutral by prior evidence).** The user asked for #25 explicitly as mode progress (not perf). Resurrected the parked xnu25-warm-ab fold (task_self/uid/gid/vchroot into the checkin reply; guest seeds its caches from the reply instead of separate task_self_trap/uidgid/vchroot_path round-trips) onto the post-collapse base: the 8 touched files (generate-rpc-wrappers.py, ciderd rpc_wire.rs/handler.rs, guest loader main/rpc/stack.rs, xnu patches 0045/0046) were unchanged since the fold's parent, so `jj duplicate` applied the delta cleanly; the generator<->wire pair was already Pass-123-validated byte-identical. Min-prefix build: rc=0 in ~29min (vs Pass 123's ~5.5h full-closure) -- sourceGroups (#54) makes it incremental now even though libsystem_kernel (which everything links) changed. GATE on the built prefix: correctness ALL PASS (shell 6*7=42, uid 0, exec true 10/10, exec false->rc1, 8-way parallel 8/8); RPC-count A/B vs the collapse-only rt PROVES the fold: per-spawn total 43->40, with task_self 3->2, uidgid 4->3, vchroot_path 3->2 (each loses its first per-process fetch, now read from the checkin reply). The drop is modest (-3/spawn) because the mig/leaf caches already covered the repeat reads; the fold removes the first fetch. PERF: neutral -- not re-measured (Pass 123 already established within-noise, and a best-effort seed is no-regression by construction, Pass 98); landed for ARCHITECTURE (fewer guest<->ciderd round-trips = the xnu-rpc-free mode), which is what the user asked for. Committed nvyurxvr. The mode now folds the safe set {task_self, reply_port implicit, uid, gid, vchroot}; host_self/thread_self stay unfoldable (Pass 92 over-release), and the rest is Category C service IPC.

**Pass 132 (task #23/#32: in-guest Mach PORT LAYER started -- user green-lit "port layer"; ciderd-side batch reply-port RPC landed + compiles, guest pool patch is the next step).** Goal: guest handles self-IPC reply-port allocation IN-GUEST, cutting the ~6 reply_port round-trips/spawn -- the first real step toward running without xnu rpc (folding plateaus ~30/spawn; the rest are genuine IPC). DESIGN decision, grounded in XNU's name format (osfmk/mach/port.h: name = (index<<8)|gen8, index off a low free-list): a guest-mint "local name" scheme risks collision (no free bit; a high-index range 0x40000000+ is collision-free but needs a ciderd translation table -- deferred). Chose the SAFER BATCH-POOL: ciderd batch-allocates REAL reply-port names (no translation, no collision), the guest pools them and hands them out locally. CIDERD SIDE DONE + COMPILES (buck2 //src/linux/server:ciderd BUILD SUCCEEDED): new RPC mach_reply_port_batch (callnum 81, vchroot_path buffer pattern -- guest passes a uint32_t buffer + size; handler mints N reply ports via mach::mach_reply_port(), write_mem's the names into the guest buffer, returns count). Generator + regenerated wire (byte-clean via the 5th-arg regen, validated) + handler. REMAINING (guest patch 0047, then ONE nix prefix build ~30min since buck2 will not apply vendor/patches): a per-process reply pool in mach_traps.c (libsimple_rwlock-guarded like getuid.c; mach_reply_port_impl pops from the pool, refills via the batch RPC when empty, falls back to the single RPC on error) + a __reply_pool_reset() called from _mach_fork_child (mach_init.c) so a fork-WITHOUT-exec child does not reuse the parent task's ports (exec resets naturally via the fresh image); GATE hard incl. fork-without-exec cases (bash subshells / $()), since a stale-port bug there is the failure mode. Expected reply_port ~6 -> ~1-2/spawn (40 -> ~35); deallocs still RPC (a later translation step removes those). Committed the ciderd side as the resumable checkpoint.

**Pass 133 (task #23/#32: in-guest reply-port POOL works -- reply_port 6->1/spawn, buck2-proven; but the nix prefix build did NOT compile patch 0047, a sources/buck2 cache issue to bust before it lands in-prefix).** Built the full port layer: ciderd mach_reply_port_batch RPC (committed kkotvwmk) + guest patch 0047 (mach_reply_port_impl serves from a per-process pool filled by one batch RPC; __reply_pool_reset in _mach_fork_child; libsimple_rwlock-guarded; single-RPC fallback). The ~2h nix build (result-portlayer) SUCCEEDED and correctness/fork-without-exec passed, BUT the gate showed the pool INACTIVE: reply_port stayed 6, reply_batch=0, and nm of the built libsystem_kernel.dylib had ZERO reply_pool syms -- 0047's code was not in the dylib. Verified 0047 is CORRECT and applies (cumulative real apply of 0044-0047 lands 16 reply_pool refs). PROVED the code works via the fast buck2 loop: applied 0044-0047 to the working-copy vendor/src, `buck2 build //vendor/src/xnu:system_kernel_final` SUCCEEDED, swapped that dylib into the result-portlayer rt -> reply_port 6->1, reply_batch=2 (one batch per guest phase: mldr + the exec'd image), correctness rc=0, dyld runs. So the in-guest reply-port pool FUNCTIONS (guest hands out reply ports locally; net reply-alloc RPCs 6 -> 3). The nix miss is a BUILD-CACHE problem, not a code bug: nix applies vendor/patches to a fetched submodule and runs buck2; the build log for the restarted run shows NO patch phase (sources.drv was cached) and buck2 evidently reused a cached libsystem_kernel without 0047. TO LAND IN-PREFIX: force a clean rebuild (bust the buck2/sources cache) so 0047 compiles into the nix dylib, then re-gate. NEXT port-layer gains (bigger): pool the DEALLOCs (return reply ports to the pool -> cut the ~4 dealloc RPCs/spawn) and share the pool across the mldr->exec boundary (1 batch not 2). Committed 0047 (buck2-validated); ciderd side already committed.

**Pass 134 (task #23/#32: in-guest reply-port pool LANDED IN-PREFIX -- reply_port 6->0/spawn, the first per-op xnu-rpc ELIMINATION for guest execution).** The prior nix build missed 0047 because it was UNCOMMITTED (nix flakes build the committed tree; the committed ciderd RPC landed, the uncommitted patch did not). Committed 0047 (ykqkzmvo) and rebuilt (~2h, wide relink since libsystem_kernel changed): the built libsystem_kernel.dylib now has the pool (nm: reply_pool syms present), result-portlayer repointed to a NEW store path. GATE on the real prefix: correctness ALL PASS (shell 6*7=42, uid 0, exec true 10/10, false->rc1, 8-way parallel 8/8, fork-without-exec subshell+$() clean), and RPC count: reply_port 6->0, reply_batch=3, total 43->37 (collapse-only 43 -> #25 fold -3 -> pool -3). So the guest now allocates ALL its reply ports from a local pool filled by batch RPCs -- reply_port is fully in-guest (0 per-reply_port round-trips), the first genuine xnu-rpc elimination on the guest-exec hot path (self-VM was already in-guest; this is the first self-PORT op moved in-guest). The 3 batches are one per guest phase (mldr/dyld/exec'd image); the pool does not yet survive across the exec boundary. NEXT reductions: (a) pool the DEALLOCs (return pool ports to the free list instead of RPC-dealloc -> cut ~4 dealloc/spawn + enable reuse so fewer batches), with reuse-safety care (only re-pool a port whose reply cycle completed, mirroring mig_dealloc_reply_port's destroy-on-timeout); (b) share the pool across mldr->exec (fewer batches). Committed: ciderd batch RPC (kkotvwmk) + guest pool 0047 (ykqkzmvo), both now in-prefix.

**Pass 135 (task #23: reply-pool over-consumption hypothesis REFUTED; the 3 batches/spawn are 3 guest MEMORY CONTEXTS, not a broken cache; further reduction = cross-exec pool sharing, marginal).** Chased why the pool fetches 3 batches/spawn when pre-pool reply_port was only 6. Instrumented (buck2 loop): guest mach_reply_port_impl call-count + mig_get_reply_port hit/miss, and a ciderd DBG print of the batch buffer_size/cap. Findings: (1) mach_reply_port_impl is called only 3x/spawn with the pool (pattern `hhhhAmAAhhh` = 7 mig HITS / 1 miss / 3 allocs) -- IDENTICAL to pre-pool, so the MIG per-thread reply-port cache still WORKS and the pool does NOT over-consume (hypothesis refuted). (2) ciderd returns cap=8 per batch (bufsz=32) -- the batch is full-size. So the 3 batches are 3 allocations in 3 SEPARATE guest memory contexts, each with a fresh (empty) static pool: the shellspawn fork-child (post-fork _mach_fork_child -> mach_init_doit -> 1 reply port, then execs mldr), mldr (fresh memory after exec), and the exec'd target image (fresh memory). The pool is efficient WITHIN a context but cannot survive the 2 exec boundaries (fresh memory each). NET remains reply_port 6->0 replaced by 3 batches (=-3). To go 3->1 batch (-2 more) needs passing the pool's unused names across exec via apple[] (task is reused across exec, so the ports stay valid), the same mechanism #25 uses -- moderately involved (mldr change + a seed patch) for -2. Instrumentation reverted (vendor pristine, ciderd DBG print removed). The reply-port pool's main win (6->0) stands; batch reduction is the marginal remaining port-layer step.

**Pass 136 (task #23/#33: found the DOMINANT ciderd leak -- ~1 task + 1 parked microthread LEAKED per guest process via the pidfd-death path that tears nothing down; the fix is BLOCKED on guest-thread IPC teardown, which SIGSEGVs -- not landable unsupervised).** While hardening the reply-port pool, found ciderd RSS grows without bound (~0.5-0.8 MB/spawn, never plateaus) -- an OOM risk for exactly the build workloads #11 targets. ROOT CAUSE, traced with a DSERVER_TRACE_DESTROY instrument (env-gated eprintln in xnu_sys_task_destroy / xnu_sys_thread_destroy + the pidfd handler): over 10 warm `cider exec true`, task_count and parked_count grow EXACTLY +1 each per spawn and DESTROY task/thread = 0 -- nothing is ever freed. Why: a warm exec NEVER sends a real-exit checkout. Every checkout carries exec_listener_pipe >= 0 (execve, real_exit=false); the process's actual death is detected via its PIDFD ("CIDER_PROCKQ target died", ciderd.rs:416), and that handler ONLY notifies watchers (on_target_died) + closes the fd -- it calls neither prune_process nor any task/thread free. So each dead process leaks its xnu_sys task (ipc_space + vm_map = the ~500 KB heap growth the smaps diag showed) AND its persistent doWork microthread, which sits parked at the loop top forever; that thread's xnu_sys_thread holds a task_reference (thread.rs:184, xnu_sys_rs_task_reference_internal), so it pins the task's refcount and even releasing the create-ref (the Pass-committed despawn_task) cannot destroy the task. This ALSO explains why the earlier checkout-path task-free read as ~40% better: that was allocator warm-up artifact, not the task actually being freed (it never was -- DESTROY task = 0).

WIRED the real cleanup into the pidfd-death handler (discard_parked the dead process's parked microthreads, releasing their task refs, then despawn_task) -- and it CRASHES ciderd. addr2line'd the SIGSEGV (fault 0x404 = null-ptr field): discard_parked -> xnu_sys_thread_destroy -> ipc_thread_terminate -> ipc_port_dealloc_special -> ipc_port_destroy -> null deref. Destroying a GUEST thread's IPC is a path with ZERO prior coverage (guest threads were ALWAYS leaked, so xnu_sys_thread_destroy on one had never run); it null-derefs tearing down a thread special port (likely ith_special_reply_port, possibly interacting with the reply-port pool). The obvious alternative -- WAKE the parked doWork thread to reach a clean terminal state before destroy (reap_thread with real_exit) -- HANGS instead: the woken loop-exit posts an S2C reply to the already-dead guest and blocks the single-worker serve loop.

VERDICT: the leak is real, important, and now precisely characterized, but the fix is a genuine architectural gap -- cider never implemented safe guest-thread/task teardown -- landing on a crashing, IPC-correctness-critical path. After two ciderd crashes (segv, then hang) it is NOT responsible to land unsupervised. Reverted the pidfd-teardown attempt (saved: scratchpad leak-fix-attempt-pidfd.diff). Kept the safe checkout-path infra (despawn_task/unregister_task_lookup), commit message rescoped for honesty: it fires only on a real-exit checkout, which warm exec does not do, so it does not by itself reduce per-spawn RSS. RECOMMENDED next approach (needs care / likely maintainer input on guest-thread IPC invariants): (1) root-cause the ipc_port_destroy null deref on a guest thread's special port -- pool artifact, or a missing init? -- and guard/fix it; (2) OR find a teardown ORDER where the task's ipc_space_terminate (inside xnu_sys_task_destroy) reclaims the thread ports, avoiding per-thread ipc_thread_terminate entirely; (3) then reap the parked microthreads WITHOUT waking them (no S2C to a dead guest) + despawn_task, driven from the pidfd-death handler (the true process-death signal, ciderd.rs:416), not the checkout path. The DSERVER_TRACE_DESTROY instrument (task_count/parked_count/DESTROY counters) is the ground-truth gate for any retry: it must show both counts flat and DESTROY = spawns.

**Pass 137 (task #23/#33: the "blocked" ciderd leak is FIXED -- 98% cut via run_on_task; the teardown SIGSEGV was a missing current-thread context, NOT vendor IPC surgery).** Pass 136 called the pidfd-death teardown crash a vendor XNU blocker needing supervision. It was not. Instrumented ipc_thread_terminate (probe on the port values): the crashing port (kport) is a VALID pointer, and the null deref is INSIDE ipc_port_destroy at `thread_t self = current_thread(); boolean_t top = (self->ith_assertions == 0)` -- current_thread() returns NULL when the teardown runs in the bare epoll loop, so `self->ith_assertions` derefs null (fault 0x404, and the source of the "lock mutex without an active thread" warnings). The xnu-sys IPC teardown REQUIRES a current-thread context, exactly like task CREATE (registry.rs:127 "ON A MICROTHREAD, NOT THE DAEMON THREAD"). FIX: run the pidfd-death teardown on a kernel-task microthread via sched::run_on_task (the existing timer/kqchan pattern, sched.rs:527) -- discard_parked frees the parked doWork microthreads (dropped WITHOUT resuming, since a dead guest would block the doWork exit's S2C reply), then despawn_task releases the task's create-ref. GATE (300-spawn soak): per-spawn ciderd RSS growth ~900 -> ~21 kB/spawn (98% cut), task_count/parked_count FLAT (were +1/spawn), exec true 100/100, fork-heavy shell 10/10, ciderd survives, NO crash/hang. The single teardown path is now the pidfd handler; the never-firing checkout real_exit teardown (Pass-136 partial) is removed. Committed 1c5d16f7 (deepcomment'd). LESSON: Pass 136 mis-declared this blocked-on-vendor after two crashes; the addr2line frame said ipc_port_destroy, but the actual defect was one frame up (the caller's context), found only by instrumenting the port values. A crash in vendor code is not proof the fix is in vendor code.

RESIDUAL (follow-up, matters for real builds): the task STRUCT is still not destroyed (DESTROY task = 0). Each execve within a spawn (shellspawn->mldr->target reuses the task) creates a guest thread that reaps (microthread dropped) but is never xnu_sys_thread_destroy'd, so those threads pin the task's refcount and its vm_map/ipc_space leak. Small for a static `true` (~21 kB) but ~1.8 MB for a bash proc (bigger vm_map), so a fork-heavy build still accumulates. To reach ~0: destroy the execve-reaped threads too, so the refcount hits 0 and task_deallocate frees the vm_map. Cleanest = a PERSISTENT reaper microthread (one long-lived kernel-task thread that runs all teardowns) -- it supplies the current-thread context AND avoids the per-teardown scratch-thread leak run_on_task itself has (it spawns+drops a microthread, leaking its xnu_sys_thread, part of the ~21 kB). Alternatives: destroy-at-reap (adds run_on_task to the per-spawn checkout path, perf-sensitive) or walk the task_threads circular queue at death (error-prone per thread.rs:227). Gate any follow-up with DSERVER_TRACE_DESTROY (DESTROY task must = spawns) + a bash/fork-heavy RSS soak that must plateau.

**Pass 138 (task #23/#33: the ciderd per-process leak is COMPREHENSIVELY fixed -- the fork-heavy / build-workload leak eliminated via a per-process death watch).** After Pass 137's run_on_task fix (crash + 98% for exec true) + force-destroy (frees the task vm_map, commit 276844f8), the fork-heavy case (bash + subshells) STILL leaked ~1.6 MB/shell. ROOT CAUSE, traced with DSERVER_TRACE_DESTROY on a fork-heavy shell: DESTROY task fired only for the 3 top-level bash processes, not their subshells (nsid jumped 2->8->13, skipping the 3-7 subshell/command-subst children). ciderd arms a death watch (pidfd) ONLY via kqchan_proc_open -- a GUEST RPC the launcher issues for its top-level spawns. An in-guest fork (a bash subshell, a build's compiler child) is waited via waitpid, never kqueued, so ciderd never saw its death, never ran the teardown, and its task leaked. This is THE build-workload leak: real builds fork thousands of unwatched children. FIX (commit db940422): arm a pidfd death watch on EVERY process at checkin (fork children check in too, is_fork), deduped by nsid; its death handler is now the SOLE teardown trigger (run_on_task { discard_parked + prune_process + despawn_task }), and the kqchan target-died path keeps only its NOTE_EXIT delivery; pidfds close on death (no fd leak). GATE: fork-heavy soak (25 bash shells with subshell + command-subst) 25/25 correct, ciderd RSS growth ZERO (was ~1.6 MB/shell), fds stable 10->10, exec true 100/100 + 8-way parallel 8/8, ciderd survives.

THE LEAK ARC (task #33, four commits, from "blocked on vendor IPC surgery" to comprehensively fixed): (1) 1c5d16f7 -- run_on_task gives the pidfd-death teardown a current-thread context; the Pass-136 "blocked" crash was a null current_thread() in ipc_port_destroy, not vendor IPC. (2) 276844f8 -- force-destroy (xnu_sys_task_destroy) frees the task's vm_map/ipc_space, which execve-reaped threads pin against the refcount-safe task_deallocate. (3) db940422 -- watch every process, so unwatched in-guest forks are reaped too (the dominant build-workload leak). Net: exec true ~900 -> ~13 kB/spawn, fork-heavy ~1.8 MB/shell -> ~0. RESIDUAL (small, optional follow-up): the ~13 kB/spawn left on exec-true is run_on_task's OWN scratch-thread leak (it spawns+drops a microthread per teardown, leaking that microthread's xnu_sys_thread) plus the reaped thread structs force-destroy leaves behind; a PERSISTENT reaper microthread (one long-lived kernel-task teardown thread, reused for all teardowns) would take it to ~0 and remove the force-destroy thread-struct leak. Fork-heavy is already ~0, so this is not build-critical.

**Pass 139 (task #23/#33: replaced force-destroy with a refcount-safe teardown that destroys the execve-reaped threads; exec-true residual ~13 -> ~6.5 kB/spawn, no more "ignore refcount").** The force-destroy from 276844f8 freed the vm_map but leaked the execve-reaped thread structs and ignored the refcount. Better: destroy those threads properly so the refcount reaches 0 and refcount-safe task_deallocate frees the task. despawn_task now walks the task's thread list (task_threads circular queue) and xnu_sys_thread_destroy's the remnants -- discard_parked already removed the parked doWork threads (their destroy queue_removes them), so the walk finds only the execve remnants. COLLECT-then-destroy (destroy queue_removes each, which would corrupt a walk in progress), follows the element-pointer contract (the stored link IS the thread; xnu::thread::queue_enter_threads), guard-bounded against a cyclic list. Commit 013d580c. GATE: DESTROY task = spawns via task_deallocate (NOT force), exec-true soak ~6.5 kB/spawn (was ~13), 25 fork-heavy shells 25/25, 8-way 8/8, fds stable, no crash. SAFETY WIN: if the walk ever misses a thread the task is merely deferred (leaked), never freed under a live ref -- strictly safer than force-destroy. RESIDUAL now ~6.5 kB/spawn = run_on_task's OWN scratch thread (each teardown spawns+drops a microthread on the kernel task, leaking its xnu_sys_thread). To reach ~0: (a) a bounded "destroy the prior teardown's scratch thread at the start of the next" trick (safe: all teardown scratch threads live on the persistent kernel task; leaves 1 leaked at any instant), or (b) a persistent reaper microthread. Both are diminishing-returns polish (fork-heavy is already ~0, exec-true rate tiny), and (b) risks the currently-clean synchronous teardown, so left OPTIONAL. Net: the leak is 900 -> ~6.5 kB/spawn (99.3%), comprehensively fixed for build workloads.

**Pass 140 (task #23: the mach_msg lever is NOT scary Category-C -- the ~7 mach_msg/spawn are guest->KERNEL host/task special-port queries, cacheable in-guest; a reframe of Pass 129/130).** Resumed the ACTUAL task #23 (run guest without ciderd; the leak #33 was a side-quest) on the biggest remaining lever: mach_msg_overwrite #38 (~7/spawn), which Pass 129/130 called "large, delicate, Category-C service IPC, needs go/no-go." Instrumented ipc_kmsg_trace_send (DSERVER_TRACE_MSG now logs dest_pid + msgh_id/MIG routine + descriptors) and traced one warm exec true. FINDING: every mach_msg goes to to_pid=-1 (a KERNEL/special port), NOT to a guest service (launchd/notifyd). They are guest->kernel MIG request/reply pairs (id X -> reply X+100): host_info (200, static host data, no port), host_get_io_master (206, returns a port), two task special-port queries (3409/3418, return ports), plus a one-way notification (70). So this is NOT service IPC that needs the host -- it is CACHEABLE guest-local kernel state, the same Category-B fast-path class as the already-landed task_self leaf-cache / reply-port pool. Pass 130's "non-cacheable service IPC" characterization was WRONG. DESIGN: (a) host_info -> cache the static result in-guest (no port, so NO over-release risk -- the easy safe win); (b) the port-returning queries (host + task special ports) -> cache the ports in-guest WITH the "do not deallocate a cached name" guard task_self already uses (Pass 22), which defuses the Pass-92 over-release hazard; (c) notification 70 -> likely elidable. Expected mach_msg ~7 -> ~1-2/spawn (total ~37 -> ~31). Guest-side (libsystem_kernel), so it needs a vendor patch + the ~2h nix prefix build to land (buck2 guest-dylib loop to prototype). Committed the KMSG diagnostic. NEXT: prototype the host_info cache (safe), then the special-port cache with the dealloc guard, gating RPC count + correctness + fork-without-exec (the over-release failure mode).

**Pass 141 (task #23: in-guest mach_msg query cache + deallocate guard IMPLEMENTED, compiles, 0048 committed, in-prefix build running to gate).** Executed the user's "build it anyway" choice on the mach_msg lever. VALIDATED the opportunity from the SAVED KMSG trace (/tmp/cider-msg-px/ciderd.log, no new build): one warm exec true is ONE task (single task-port pointer 0xb2da9805b280 across all 3409/3418 sends; two execs would show two), and within its one shared libsystem_kernel (mldr jump-loads the target in-process, main.rs:413, NOT execve) the cacheable requests repeat to the SAME dest: 206x2, 3409x2, 3418x2, 200x1 = 7 = the doc's mach_msg_overwrite 7. So a (dest,id)-keyed replay cache dedups 3 (206/3409/3418), the differing reply port (local=) irrelevant since a hit bypasses the port mechanism. IMPLEMENTED in mach_traps.c (patch 0048, on top of 0047): mach_msg_overwrite_trap_impl caches the reply bytes of a whitelisted query (host_info 200, host_get_io_master 206, task special ports 3409/3418) and replays on a (dest,id) hit. OVER-RELEASE HAZARD (Pass 92, the reason Pass 130/131 called host_self/thread_self unfoldable): the port-returns (206/3409/3418, COMPLEX reply ndesc=1 dtype=0) hand back a real send right only on the FIRST call; a replayed hit adds no ref, so a per-call deallocate would over-release. GUARD (task_self's Pass-22 trick): the store parses the reply port descriptor, records the name, and _kernelrpc_mach_port_deallocate_trap_impl swallows deallocate of a guarded name (leaks 1 uref reclaimed at task exit, keeps the port valid). host_info (200, non-complex, no port) is inherently safe. Compiles via buck2 //vendor/src/xnu:system_kernel_final. Committed 0048 (1b0ba86a, bookmark mach-msg-cache). BUILD-INFRA scars this pass: the buck2 working-tree pin was un-patched/inconsistent (fork.c 3-arg vs 6-arg checkin); restored via `FORCE=1 scripts/buck-src.nu` + `cider-src-normalise` (must run inside nix develop). Measurement PENDING: nix build .#cider-buck2-prefix-min running (~2h, wide relink since libsystem_kernel changed). GATE plan (scratchpad/measure-mm.sh): A/B RECV #38 count baseline /tmp/cider-msg-rt (expect 7) vs my rt (expect ~4), + correctness (exec true 10/10, false->rc1, shell 42, 8-way, fork-without-exec = the over-release mode). Rootless run (euid 1000, no sudo): CIDER_NO_LAUNCHD=1 DSERVER_LIBEXEC_PATH=<out>/cider_prefix_min__prefix/libexec/cider.

**Pass 141 RESULT (measured in-prefix, build afynh6sq...-cider_prefix_min-out, nm-confirmed my cache+guard syms in libsystem_kernel).** A/B one warm `cider exec true`, DSERVER_TRACE_CALLS RECV #38 (mach_msg_overwrite): baseline /tmp/cider-msg-rt = 7, my cache = 5 (-2/spawn). CORRECTNESS ALL PASS on the cache build: exec true 10/10, false->rc1, shell 6*7=42, 8-way parallel 8/8, and fork-without-exec = 'sub-42' -- the last is the over-release failure mode, so the deallocate GUARD works (no over-release; the port-return cache is safe). KMSG (DSERVER_TRACE_MSG) breakdown of what still reaches ciderd with the cache: 200x1 (no repeat, saves 0), 206x2 (NOT deduped), 3409x1 + 3418x1 (deduped from 2, the -2). WHY 206 is not deduped: host_get_io_master goes to the host port, and mach_host_self() -> host_self_trap() RPCs a FRESH host-port name every call (mach_legacy.c:34, and the 0044 note "host_self is NOT cacheable this way: adds a send-right ref each call"), so the two 206 sends carry different dest names and the (dest,id) key misses; the task ports 3409/3418 use the cached task_self name (stable) so they dedup. So the mach_msg cache lands at 7->5 (per-spawn total ~37->35), perf-null as expected, ARCHITECTURE progress (2 fewer guest->ciderd RPCs), the FIRST mach_msg round-trips ever removed from guest exec. FOLLOW-UPS (both need another ~2h build, marginal/perf-null): (a) id-only keying for the host ids 200/206 (one host, so dest-independent is safe with the io_master guard) would dedup 206 -> 7->4; (b) caching host_self itself with the same dealloc-guard trick (the Pass-92 "unfoldable" port) would drop host_self_trap (#34, 3/spawn) AND stabilize the host name so 206 dedups via (dest,id) -- a bigger ~3-RPC win but riskier (host_self is called broadly; the exec-true gate does not stress IOKit/host users). Committed 0048 (mach-msg-cache).

**Pass 142 (task #23: in-guest host-self cache LANDED, 0049; and the guard's real payoff is swallowing the host port's per-call deallocs). Combined 0048+0049 = 37->31 RPCs/spawn (-6, ~16%).** Built .#cider-buck2-prefix-min (915ham9s...), nm-confirmed g_host_self_cached in libsystem_kernel. 3-way A/B, DSERVER_TRACE_CALLS, one warm exec true, TWO rounds each (stable, identical): baseline total=37 (#34 host_self=3, #38 mach_msg=7, #39 dealloc=4) -> 0048 only total=35 (#38 7->5) -> 0048+0049 total=31 (#34 3->2, #39 4->1). ATTRIBUTION: 0048 = -2 (mach_msg dedup; its guard on io_master/task-special saved 0 deallocs because the guest does not deallocate those). 0049 = -4: host_self_trap 3->2 (-1, per-context cache dedups the target phase's repeats) PLUS dealloc 4->1 (-3) -- the BIG one -- because mach_host_self adds a send-right ref every call that callers DEALLOCATE (the exact Pass-92 ref-per-call the 0044 note flagged), so the deallocate guard swallows ~3 host deallocs/spawn. So the guard mechanism's real payoff landed on host_self, not the mach_msg port-returns. CORRECTNESS hardened: exec true 50/50, 16-way parallel 16/16, fork-heavy (subshell+cmd-subst) 10/10, a host-using shell (id -u + sysctl hw.ncpu = '0:80', both correct via the cached host port), ciderd survives, no crash. 206 (host_get_io_master) STILL does not dedup: host_self is per-libsystem_kernel-instance, and a warm spawn has TWO contexts (shellspawn fork-child + the exec'd mldr/target) with DIFFERENT host names each; task_self's 3409/3418 dedup only because #25 SEEDS task_self across exec (stable name in all contexts). FOLLOW-UP (would complete it, now UNBLOCKED by the guard): add host_self to the #25 checkin-reply seed (Pass 131 excluded it as "unfoldable" over Pass-92 over-release, which the guard now defuses) -> stable host name cross-context -> 206 dedups AND host_self_trap -> ~1. Committed 0049 (e7b8821e). Net xnu-rpc-free trajectory: 43 -> 40 (#25) -> 37 (pool) -> 31 (mach_msg cache + host_self guard), all perf-null architecture, gate+soak clean.

**Pass 143 (task #23/#36: host_self checkin-seed LANDED, 0050 + wire; 206 dedups, total 31->30. GATED VIA THE BUCK2 DEV-LOOP SWAP, no 2h nix build -- the faster path).** User asked "why nix when you can test with buck2?" -- right, and it is what cider-buck2-dev-loop says. The local buck2 FULL prefix build is still blocked on an SDK symlink (root//darwin/.../MacOSX.sdk/.../notify.defs, which nix's cider-src assembles but --all does not), but the dev-loop does NOT need it: buck2 build the 3 changed components (//vendor/src/xnu:system_kernel_final, //src/linux/server:ciderd, //src/darwin/loader:mldr -- all already compiled this session, instant --show-output), then swap them into a chmod-writable cp of the last-good prefix (915ham9s 0048+0049) at libexec/cider/usr/lib/system/libsystem_kernel.dylib + bin/ciderd + usr/libexec/cider/mldr. nm confirmed __host_self_trap_set in the swapped dylib. IMPLEMENTATION (8 files, all compiled green): generate-rpc-wrappers.py adds host_self to the checkin reply; rpc_wire.rs REGENERATED via the generator's optional 5th arg (the committed "auto-generated -- DO NOT EDIT" Rust mirror is NOT rebuilt at buck2 build time, unlike the guest C rpc.h which IS -- that is why ciderd first failed E0560 no-field-host_self); ciderd handler fills it via mach::host_self_trap(); loader rpc.rs/main.rs/stack.rs carry it to apple[] (dserver_host_self=); guest 0050 = lkm.c parses it -> __host_self_trap_set (seeds g_host_self_cached + guards the name). RESULT (3-way A/B, stable): baseline total=37 -> 0048+0049 31 -> +seed 30. The -1 is #38 mach_msg 5->4: host_get_io_master (206) now DEDUPS (the uncertain part worked -- the seeded host name is stable across the fork-child/target contexts). host_self_trap #34 stayed 2 (NOT ->1): those fetches happen in EARLY init before mach_driver_init applies the apple[] seed, so the seed only stabilizes the name for the later 206 calls, it does not remove the early fetch. Correctness+soak clean: exec true 100/100, false->rc1, shell 42, 8/16-way, fork-heavy 20/20, no ciderd leak (RSS growth in-range with baseline), ciderd survives. perf-null. Committed 0659eef6 (mach-msg-cache). NET xnu-rpc-free arc this session: 37 -> 30 RPCs/spawn (0048 -2, 0049 -4, 0050 -1), all gated, all perf-null architecture. host_self_trap #34 (2) + thread_self #35 (3) remain, needing an EARLIER seed (pre-mach_driver_init) or in-guest thread cache -- diminishing.

**Pass 144 (task #24: JSC arm64 LLInt/WASM offline-asm link ROOT-CAUSED by static inspection -- a missing arm64 LLInt object; two fix paths, both need an aarch64 host to compile+verify).** Investigated the long-standing #24/#14 blocker (undefined `_vmEntryToJavaScript` / `_wasm_entry` / `_wasmLLIntPCRange*` at the arm64 JavaScriptCore link) instead of leaving it as "needs a heavy build." EVIDENCE (all on this x86_64 host, no build): (1) `JavaScriptCore_dylib.objs` (vendor/src/BUCK:62470-62478) is a FLAT list whose only LLInt members are `:low_level_interpreter_x86_64_obj` + `:low_level_interpreter_i386_obj` (plus JavaScriptCore_obj/obj2) -- there is NO arm64 sibling. (2) `grep low_level_interpreter_(arm64|aarch64)` in the BUCK = 0 hits; the only LLInt targets are i386 + x86_64. (3) Those targets carry the header "GENERATED from the reference build.ninja by gen-buck-from-ninja (deleted)" (BUCK:60603-60605): the cmake->ninja->buck lowering only ever ran for i386/x86_64, so no arm64 LLInt target was emitted. (4) The committed offline-asm DERIVED SOURCES exist only for `LLIntOffsets/{C_LOOP,X86_64}/{release,debug}/LLIntAssembly.h` -- there is NO `ARM64/` directory. (5) BUT the ARM64 backend `offlineasm/arm.rb`, the sources `llint/LowLevelInterpreter.asm` + `llint/WebAssembly.asm`, and the full C_LOOP path (`llint/LLIntCLoop.cpp` + committed `C_LOOP/` LLIntAssembly.h) are all present. CONCLUSION: arm64 JSC was simply never given a native ARM64 LLInt/WASM offline-asm object; the pre-generated LLInt objects cover only i386/x86_64, so the arm64 dylib link finds the offline-asm entry points undefined. This is NOT a compiler/toolchain bug -- it is a missing generated target.

TWO FIX PATHS (each must be executed AND gated on an aarch64 machine -- `scripts/buck-setup.nu:249` pins guest_arch to the host `uname`, so this x86_64 session builds x86_64 JSC, which links fine; and the native path's offset-extractor is arch-dependent, so it needs the arm64 toolchain):
- (A) NATIVE ARM64 (matches i386/x86_64, fast interpreter): generate `LLIntOffsets/ARM64/{release,debug}/LLIntAssembly.h` (+ the WASM equivalent) by running offlineasm (`offlineasm/asm.rb`, backend arm.rb) against LowLevelInterpreter.asm/WebAssembly.asm, driven by an ARM64-compiled LLIntOffsetsExtractor; add a `low_level_interpreter_arm64_obj` target mirroring `low_level_interpreter_x86_64_obj` (compile LowLevelInterpreter.cpp + the WASM LLInt TU against the ARM64 headers); add `:low_level_interpreter_arm64_obj` to `JavaScriptCore_dylib.objs`. The darwin_dylib rule already lists both x86_64 and i386 LLInt objs unconditionally and slices per-arch, so an arm64 entry composes the same way.
- (B) C_LOOP (expedient bring-up, portable C++ LLInt, no native offline-asm): build arm64 JSC with `ENABLE(C_LOOP)` so `vmEntryToJavaScript` et al. are ordinary C++ from LLIntCLoop.cpp + the already-committed `C_LOOP/` LLIntAssembly.h; ensure WASM is C_LOOP'd or disabled so `_wasm_entry`/`_wasmLLIntPCRange*` resolve. Gets a linking JavaScriptCore_dylib fastest for FULL-prefix bring-up at an interpreter-perf cost, upgradeable to (A) later. NOTE #24 stays NON-GATING for cider's actual goals: bash/nix and the min/framework prefix do not need JSC (Passes 56/117), so this only unblocks the FULL-prefix milestone, not #23/#39. DEEPER ROOT (already in docs/fully-without-ciderd.md "Task #24" section, session 3): the missing BUCK obj traces to `JavaScriptCore/CMakeLists.txt` (which the BUCK is generated from) having low_level_interpreter cases for x86_64 (X86_64 header) + i386 (C_LOOP header) but NO arm64 case (~lines 2160-2181), so on arm64 LowLevelInterpreter.cpp is compiled as the `darling/source/empty.c` stub and the entry symbols are never defined -- fix the CMakeLists.txt arm64 case, not the generated BUCK by hand. A BOUNDED verify exists (no full arm64 dylib link): compile LowLevelInterpreter.cpp with the chosen arm64/C_LOOP header, `llvm-nm` the obj, confirm the entry symbols are DEFINED.
