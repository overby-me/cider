...
//! through darwin/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX.sdk/usr/include, a
...
//!
//! Usage:
//!   cider-sdk-header-roots <namespace> ...                 > buck-src/sdk_headers.bzl
...
cider-sdk-header-roots: derive Buck2 header roots from the committed SDK symlink farm.
  ...
  cider-sdk-header-roots <namespace> [<namespace> ...]  > buck-src/sdk_headers.bzl
  cider-sdk-header-roots --list-pins <namespace> ...     # which pins are needed
";
...
impl Ctx {
    ...
    /// pins/xnu/osfmk/mach/boolean.h becomes xnu/osfmk/mach/boolean.h, or None when the file is
    /// not ours to map: committed repo content outside pins, or one of the committed trees under
    /// pins that never appear in buck-src.
    fn to_buck_src(&self, repo_rel: &str) -> Option<String> {
        let buck_rel = repo_rel.strip_prefix("pins/")?;
        ...
        for (dirpath, dirs, files) in all {
            ...
            for name in names {
                ...
                if !is_symlink(&path) {
                    ...
                    // package, so it is reported separately rather than mapped into buck-src.
...
fn main() -> ExitCode {
    ...
    if argv.iter().any(|a| a == "--framework-roots") {
        for (tree, label) in
            [("darwin/framework-include", "framework"), ("darwin/framework-private-include", "framework_private")]
        {
    ...
    if argv.iter().any(|a| a == "--pin-roots") {
        ...
        if apply {
            println!("# sdk_env deps for darwin/BUCK:");
            for lb in &labels {
                println!("        \"{lb}\",");
            }
        } else {
    ...
    if argv.iter().any(|a| a == "--repo-roots") {
        ...
        for ns in &namespaces {
            ...
            for (include_path, repo_rel) in walked {
                ...
                let n = if repo_rel.starts_with("pins/") { 3 } else { 2 };
    ...
    println!("# (darwin/Developer/.../MacOSX.sdk/usr/include), which is the authority on how");
    ...
    println!("#   cider-sdk-header-roots {} > buck-src/sdk_headers.bzl", namespaces.join(" "));
    println!();
