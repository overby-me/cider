...
impl Ctx {
    ...
    fn in_tree_target(&self, link: &str, target: &str) -> Option<String> {
        ...
        if !resolved.starts_with(&format!("{}/", self.repo)) {
            ...
            // darwin/... from the repo root). Recover the intent by resolving what follows the
            ...
            if !lexists(&cand) {
                // Task #68 moved the guest trees under darwin/, and a pin written before that
                // still says Developer/... from the repo root.
                cand = normpath(&join2(&join2(&self.repo, "darwin"), &tail));
            }
            if !lexists(&cand) {
                return None;
            }
