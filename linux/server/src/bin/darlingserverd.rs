//! The darlingserver daemon entry point: the Rust equivalent of darlingserver.cpp's
...
use darling::kqchan::{MachPortKqchan, ProcKqchan};
use darling::registry::Registry;
use darling::rpc_io::{Message, PeerAddr};
use darling::rpc_wire;
use darling::s2c;
use darling::sched;
use darling::server::Listener;
...
unsafe fn process_one_call(mb: &Rc<RefCell<Mailbox>>, handler_ptr: *mut Handler, host_pid: c_int) -> bool {
    ...
    if let Some(ref r) = reply {
        if r.len() >= 8 && i32::from_ne_bytes([r[4], r[5], r[6], r[7]]) == rpc_wire::ENOSYS {
            eprintln!("darlingserver: UNIMPLEMENTED call {} (#{})", call.call_name().unwrap_or("?"), ch.number);
        }
    }
...
extern "C" fn crash_handler(sig: c_int, info: *mut libc::siginfo_t, ctx: *mut c_void) {
    let msg = b"darlingserver: FATAL host signal ";
...
unsafe fn run(cfg: Config) -> ! {
    ...
    if libc::getuid() != 0 {
        ...
        eprintln!("darlingserver: warning: not running as root; container setup will fail");
    }
    ...
    container::enter_mount_namespace().expect("darlingserver: enter mount namespace");
    container::mount_prefix_overlay(&cfg.prefix, &cfg.libexec_path)
        .expect("darlingserver: mount prefix overlay");
    ...
    let init_pid = container::spawn_init_in_pid_namespace(&cfg, child_wait[0])
        .expect("darlingserver: clone guest init");
    ...
    let listener = Listener::bind(&sock_path).expect("darlingserver: bind RPC socket");
    ...
    eprintln!("darlingserver: container up (init pid {init_pid}); serving {sock_path}");
    ...
    loop {
        ...
        if trace_calls() {
            eprintln!("darlingserver: epoll wakeup ({n} events)");
        }
        for ev in &events[..n as usize] {
            let fd = ev.u64 as RawFd;
            if init_pidfd >= 0 && fd == init_pidfd {
                // Container init died: the session is over.
                eprintln!("darlingserver: container init (pid {init_pid}) died -> daemon exit");
                libc::_exit(0);
...
unsafe fn handle_call(
    ...
    if trace_calls() {
        ...
        eprintln!(
            "darlingserver: RECV #{} nsid={} tid={} known_thread={} prev_call_still_queued={}",
