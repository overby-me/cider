...
//! wrappers over the xnu-sys's xnu_sys_*_trap functions, which act on the CURRENT
...
//   the port traps              generated xnu_sys wrappers that call the XNU _kernelrpc_*_trap on
...
//   xnu_sys_mach_msg_overwrite    the mach_msg trap. msg and rcv_msg are GUEST addresses: the
...
use crate::xnu::traps::{
    xnu_sys_host_self_trap, xnu_sys_mach_reply_port, xnu_sys_task_self_trap, xnu_sys_thread_self_trap,
};
use crate::xnu::traps_generated::{
    xnu_sys_mach_msg_overwrite, xnu_sys_mach_port_allocate, xnu_sys_mach_port_deallocate,
    xnu_sys_mach_port_mod_refs, xnu_sys_mach_port_type,
};
...
pub unsafe fn port_allocate(target: u32, right: i32, name_out: u64) -> i32 {
    xnu_sys_mach_port_allocate(target, right, name_out)
}
...
pub unsafe fn port_deallocate(target: u32, name: u32) -> i32 {
    xnu_sys_mach_port_deallocate(target, name)
}
...
pub unsafe fn port_type(target: u32, name: u32, ptype_out: u64) -> i32 {
    xnu_sys_mach_port_type(target, name, ptype_out)
}
...
pub unsafe fn port_mod_refs(target: u32, name: u32, right: i32, delta: i32) -> i32 {
    xnu_sys_mach_port_mod_refs(target, name, right, delta)
}
...
pub unsafe fn msg_overwrite(
    ...
    xnu_sys_mach_msg_overwrite(msg, option, send_size, rcv_size, rcv_name, timeout, priority, rcv_msg)
}
