...
//!
//! **`DTAPE_FATAL_STUBS` is a build-time choice, so the C makes it.** It comes through the
...
//! `DTAPE_LOG_SAFE_STUBS` once at load. A Rust `.init_array` entry in an *rlib* is exactly the
...
use crate::bindings::{
    self, boolean_t, dtape_rs_host_consts_DTAPE_RS_DTAPE_FATAL_STUBS, expired_task_statistics_t,
...
/// `DTAPE_LOG_SAFE_STUBS`, read once. See the module note on why this is not a constructor.
fn log_safe_stubs() -> bool {
    static CACHED: OnceLock<bool> = OnceLock::new();
    *CACHED.get_or_init(|| unsafe { !getenv(b"DTAPE_LOG_SAFE_STUBS\0".as_ptr() as *const c_char).is_null() })
}
...
pub unsafe extern "C" fn dtape_stub_log(
    ...
    let (log_level, do_abort, kind_info) = if safety == 0 {
        (
            bindings::dtape_log_level_t::dtape_log_level_warning,
            dtape_rs_host_consts_DTAPE_RS_DTAPE_FATAL_STUBS != 0,
            "",
        )
    } else if safety < 0 {
        (
...
pub unsafe extern "C" fn proc_get_effective_thread_policy(
    ...
    crate::dtape_stub!("unimplemented flavor");
    0
}
...
/// The C ends in `dtape_stub_unsafe()`, which logs and then `__builtin_unreachable()`. The Rust
