...
'
  ...
  connectivityTestScript = pkgs.writeShellScript "cider-builder-test" ''
    set -euo pipefail
...
in
{
  options.services.cider-builder = {
    ...
    sshKeyPath = lib.mkOption {
      type = lib.types.str;
      default = "/etc/nix/cider-builder-key";
      description = ''
    ...
    prefixPath = lib.mkOption {
      type = lib.types.str;
      default = "/var/lib/cider-builder";
      description = ''
        Path to the Darling prefix used by the builder.  This is a
        separate prefix from the default (~/.cider) to avoid
    ...
    installNix = lib.mkOption {
      ...
      description = ''
        ...
        using scripts/install-nix-in-cider.nu before enabling the builder.
  ...
  config = lib.mkIf cfg.enable {
    ...
    programs.cider.package = cfg.package;

    # ── SSH key generation ───────────────────────────────────────────
    # Generate the ed25519 keypair on system activation if it doesn't
    # already exist.  The key is owned by root with mode 600.
    system.activationScripts.cider-builder-keys = lib.stringAfter [ "users" ] ''
      if [ ! -f "${cfg.sshKeyPath}" ]; then
        echo "[cider-builder] Generating SSH keypair at ${cfg.sshKeyPath} ..."
        mkdir -p "$(dirname "${cfg.sshKeyPath}")"
        ${pkgs.openssh}/bin/ssh-keygen -t ed25519 -N "" -f "${cfg.sshKeyPath}" -C "cider-builder@$(hostname)"
    ...
    systemd.services.cider-builder = {
      ...
      serviceConfig = {
        ...
        ExecStartPre = [
          ...
          "+${pkgs.writeShellScript "cider-builder-install-nix" ''
            ...
            echo "[cider-builder] Downloading Nix $nix_version ..."
            curl -fsSL -o "$tmpdir/nix-installer.tar.xz" "$installer_url"

            echo "[cider-builder] Extracting ..."
            ...
            if [ -z "$installer_dir" ]; then
              echo "[cider-builder] ERROR: Could not find extracted installer directory"
              exit 1
            fi
            ...
            rm -rf "$prefix_tmp"
            echo "[cider-builder] Nix installation complete."
          ''}";

        ExecStart = "${cfg.package}/bin/cider --prefix ${cfg.prefixPath} shell /usr/sbin/sshd -D -f /etc/ssh/sshd_config";

        # Stop sshd cleanly, then shut down the Darling prefix
        ExecStopPost = "-${cfg.package}/bin/cider --prefix ${cfg.prefixPath} shutdown";
    ...
    environment.systemPackages = [
      (pkgs.writeShellScriptBin "cider-builder-test" ''
