...
def main [--src: string = "", --pins: string = "", --only: string = ""] {
    ...
    # The farm names entries by sanitised path, so src/external/libdispatch is
    ...
    mut checked = 0
    for e in $entries {
        # src-external-X -> src/external/X. Only the first two segments are directories in
        # every entry the manifest carries, so rebuilding the path is unambiguous.
        let rel = ($e | str replace 'src-external-' 'src/external/')
