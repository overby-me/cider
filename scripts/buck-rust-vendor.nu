#!/usr/bin/env nu
# Materialize the locked Rust crate sources into vendor/rust/ for the Buck2 port.
#
...
# same arrangement scripts/buck-src.nu gives the pinned C sources under vendor/src/.
#
...
def main [] {
    ...
    for crate in (glob $"($vendor)/*") {
        ...
        if not ($"vendor/rust/($name)/Cargo.toml" | path exists) {
            rm -rf $"vendor/rust/($name)"
            mkdir $"vendor/rust/($name)"
            ^cp -a --reflink=auto $"($crate)/." $"vendor/rust/($name)/"
            ^chmod -R u+w $"vendor/rust/($name)"
            do -i { rm -f $"vendor/rust/($name)/.cargo-checksum.json" }
        }
        $n = $n + 1
    }
    print $"vendor/rust: ($n) crate\(s\) materialized"
}
