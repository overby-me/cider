...
def main [--all] {
    ...
    let elf_sonames = [
        ...
        libwayland-client.so libwayland-cursor.so libxkbcommon.so
    ...
    # WAYLAND, for the GUI backend (#112). wayland-scanner turns the protocol XML into C, the
    # same shape as every other codegen tool this build drives, so both paths are resolved here
    # rather than hardcoded in a rule. Only the EXTENSIONS need generating: libwayland-client
    # already exports the core wl_ interfaces.
    print "wayland: resolving wayland-scanner and the xdg-shell protocol"
    let wl_scanner = (do -i { ^nix build "nixpkgs#wayland-scanner" --no-link --print-out-paths }
...
"
    ...
    # THE GUEST ARCH FOLLOWS THE MACHINE (docs/plan-aarch64.md, D2): an aarch64 host builds
    # arm64 Mach-O, an x86_64 host builds x86_64 Mach-O. The committed default in
    # buck/toolchains/BUCK is x86_64, so this key is what flips a checkout to arm64, and
    # deleting it flips it back. Cross-building the OTHER arch is a hand edit of this file.
    let guest_arch = (if (^uname -m | str trim) == "aarch64" { "arm64" } else { "x86_64" })
    print $"guest arch: ($guest_arch)"
...
($darwin_rust_conf)($wayland_conf)elf_lib_dirs = ($elf_lib_dirs | str join ':')
