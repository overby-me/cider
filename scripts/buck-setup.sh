...
#
# Absolute paths to prebuilt tools the Buck2 build drives. They are nix store
# paths (immutable), so this file only needs regenerating when the derivation
# that produces them changes.
[darling]
