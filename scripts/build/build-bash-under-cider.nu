...
# build-hello-under-cider.nu; bash is a much larger configure/make and
...
# and cider stubbed, no container and no network: a successful build, a run whose run_rc is
...
def main [] {
    ...
    let prefix = ($env.CIDERPREFIX? | default ($env.HOME | path join ".dbash"))
...
echo "=RUN="; ./bash -c 'echo BASH_RUNS_OK; x=2; echo sum=$((x+3)); for i in a b c; do printf "%s" "$i"; done; echo'; echo "run_rc=$?"'##
    ...
    for i in 1..$retries {
        ...
        with-env {CIDERPREFIX: $prefix} {
            do -i { ^timeout 2400 $cider shell sh $gbuild out+err> $log }
        }
