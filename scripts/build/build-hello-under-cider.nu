#!/usr/bin/env nu
# build-hello-under-cider.nu: build GNU hello 2.12.3 from source *inside*
...
#
# Env overrides: DARLING (cider binary), CIDERPREFIX (short prefix path),
...
# interpolations. Verified against the bash version with nix, nix copy and the cider binary
...
def main [] {
    ...
    let prefix = ($env.CIDERPREFIX? | default "/tmp/dhb")
...
echo "=RUN="; ./hello; echo "hello_rc=$?"'
    ...
    mut reached = false
    for i in 1..$retries {
        for n in [ciderd mldr] { do -i { ^pkill -9 -x $n } }
        sleep 1sec
        let log = (mktemp --tmpdir-path /tmp dhb-out.XXXXXX)
        with-env {CIDERPREFIX: $prefix} {
            do -i { ^timeout 1500 $cider shell sh $gbuild out+err> $log }
        }
