...
#
# Generalises scripts/build/build-hello-bypass.nu (the M1 reference) to an arbitrary attr via the
...
# scripts/checks/buck-nix-bash-check.nu's job and which drives this.
#
...
#
# Usage:
#   scripts/build/build-pkg-bypass.nu <attr> [binname] [--mono <cider-store-path>] [--prefix <dir>]
# e.g.  scripts/build/build-pkg-bypass.nu hello hello
#       scripts/build/build-pkg-bypass.nu pv pv
...
def main [
    ...
    let prefix = if ($prefix | is-not-empty) {
        ...
        $env.CIDERPREFIX? | default "/tmp/cider-pkg"
    }
