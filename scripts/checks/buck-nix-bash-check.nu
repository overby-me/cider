...
# scripts/checks/buck-bash-check.nu: that one boots the container and runs the bash the port itself
...
#
# Usage:  scripts/checks/buck-nix-bash-check.nu [<attr> [<binary>]]
# Default attr is bash. Any nixpkgs x86_64-darwin attr works, since the driver underneath
# (scripts/build/build-pkg-bypass.nu) is generic.
...
def main [attr?: string, bin?: string] {
    ...
    let prefix = ($env.CIDERPREFIX? | default $"/tmp/cider-nixpkg-(^id -u | str trim)")

    if not ($"($rt)/bin/cider" | path exists) {
        say $"no materialized prefix at ($rt)"
        say "run scripts/checks/buck-bash-check.nu first -- it builds //buck/prefix:cider_prefix and"
    ...
    # relocatable prefix cannot, so they are passed here (same as scripts/checks/buck-bash-check.nu).
    ...
    do -i { ^./scripts/build/build-pkg-bypass.nu $attr $bin --mono $rt --prefix $prefix out+err> $log }
