...
# the effect was 143 dangling links. For that question use scripts/checks/buck-escape-check.nu:
...
# default now. scripts/checks/buck-pins-tree-check.nu is the check for that property, since this one
...
#
# Usage:
#   scripts/checks/buck-pin-store-check.nu                          # build both sides, compare all
#   scripts/checks/buck-pin-store-check.nu --src <cider-src path> # compare against a given tree
#   scripts/checks/buck-pin-store-check.nu --pins <farm path>
#   scripts/checks/buck-pin-store-check.nu --only libdispatch       # one pin, for a quick loop
