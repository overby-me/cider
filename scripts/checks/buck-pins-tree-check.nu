...
# the DEFAULT endpoint, and scripts/checks/buck-pin-store-check.nu passed anyway, because it compares
...
#
# Usage:
#   scripts/checks/buck-pins-tree-check.nu                    # build pinsTree, compare against cider-src
#   scripts/checks/buck-pins-tree-check.nu --tree <path>      # check a tree already built
...
def dangling [dir: string] {
    let out = (do -i { ^nu scripts/checks/buck-escape-check.nu resolve $dir } | complete)
