...
#
# Usage:
#   scripts/checks/buck-quick-check.nu                        # build the canonical target, count
#   scripts/checks/buck-quick-check.nu --attr .#some-package  # any flake attr
#   scripts/checks/buck-quick-check.nu --probe <src file>     # + edit/rebuild/revert, the cascade test
#   scripts/checks/buck-quick-check.nu --probe <f> --expect-zero   # exit 1 if anything rebuilt
#   scripts/checks/buck-quick-check.nu --probe <f> --revert-only   # strip a probe left by a kill
#   scripts/checks/buck-quick-check.nu stall                  # is a running build actually stalled
#
...
# src/darwin/frameworks/AVFoundation/constants.m: 5 builders, 97 seconds, no graph and no ld64.
...
def main [
    ...
    if ($probe | is-not-empty) {
        ...
        if (has-probe $probe) {
            ...
            say "Revert it first: scripts/checks/buck-quick-check.nu --probe <path> --revert-only"
            exit 2
        }
    }
