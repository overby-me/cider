...
#
# Usage:
#   scripts/checks/buck-runtime-check.nu              # the eleven fast checks
#   scripts/checks/buck-runtime-check.nu --with-nix   # plus buck-nix-bash-check, which is slow
#   scripts/checks/buck-runtime-check.nu <name>...    # just these, by bare name
...
def main [...names: string] {
    ...
    for c in $checks {
        ...
        # THE CHECKS LIVE IN scripts/checks/ NOW, and this dispatches by BARE NAME, so it is
        # the one place the subdirectory has to be spelled out. Flat stays as a fallback so a
        # script that has not moved, or moves back, still resolves.
        let nu_path = (if ($"scripts/checks/($c).nu" | path exists) { $"scripts/checks/($c).nu" } else { $"scripts/($c).nu" })
        let sh_path = (if ($"scripts/checks/($c).sh" | path exists) { $"scripts/checks/($c).sh" } else { $"scripts/($c).sh" })
