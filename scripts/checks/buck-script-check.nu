...
#
#   scripts/checks/buck-script-check.nu                  # 1,474 labels, plus the controls
#   scripts/checks/buck-script-check.nu --no-controls
#
# BUILDS NOTHING beyond the graph and its specs. A couple of minutes.

# EXPORTED, so scripts/checks/buck-names-check.nu can IMPORT these two rather than keep a third copy.
...
# (src/linux/buildtools/graph-specs/src/main.rs) and a check cannot import a binary. Each is
