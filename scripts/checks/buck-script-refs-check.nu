...
#
# Usage:
#   scripts/checks/buck-script-refs-check.nu              # the docs, configs and scripts
#   scripts/checks/buck-script-refs-check.nu --scan a,b   # just these files (for testing)
...
def main [--scan: string = ""] {
    ...
    let driver = "scripts/checks/buck-runtime-check.nu"
    ...
    let missing_checks = (
        # BARE NAMES RESOLVE IN scripts/checks/ FIRST, then flat, which is exactly what the
        # driver itself does. Checking only the flat form made every one of the 41 moved checks
        # look retired the moment scripts/ was split.
        $named | where {|n| not (($"scripts/checks/($n).nu" | path exists) or ($"scripts/checks/($n).sh" | path exists) or ($"scripts/($n).nu" | path exists) or ($"scripts/($n).sh" | path exists)) }
    )
    ...
    for m in $missing_checks {
        say $"FAIL: ($driver) runs ($m), and it is in neither scripts/checks/ nor scripts/"
    }
    if ($bad | is-not-empty) {
