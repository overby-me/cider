...
# and a container, which scripts/checks/buck-bash-check.nu already produces. So this drives the
...
#
# Usage:  scripts/checks/buck-smoke-check.nu
# Run scripts/checks/buck-bash-check.nu first -- this drives the root it materializes.
#
...
def main [] {
    ...
    # capped at 108 bytes.
    let prefix_dir = ($env.CIDERPREFIX? | default $"/tmp/cider-smoke-(^id -u | str trim)")
