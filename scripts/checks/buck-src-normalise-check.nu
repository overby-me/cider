...
# (src/linux/buildtools/src-normalise) and there is nothing to import, so the rules are checked
...
#
# SELF CONTAINED. It builds its own vendor/pins/ and src/darwin/ so it does not depend on which pins happen
# to be materialized on this machine, which is what made the earlier by-hand version fragile.
#
#   scripts/checks/buck-src-normalise-check.nu
...
const EXPECT = [
    ...
    # escapes to a path that exists only under src/darwin/, then through the farm link there
...
def build_repo [repo: string] {
    ...
    # The SDK farm: a link under src/darwin/ pointing into vendor/pins/, which is what makes an escaping
    # link recoverable at all.
    mkdir $"($repo)/src/darwin/sdk-farm"
    ^ln -s "../../../vendor/pins/somepin/include/real.h" $"($repo)/src/darwin/sdk-farm/link.h"
    mkdir $"($repo)/src/darwin/Developer/SDK/usr/include"
    ^ln -s "../../../../../../vendor/pins/somepin/include/real.h" $"($repo)/src/darwin/Developer/SDK/usr/include/notify.defs"
    ...
    ^ln -s "../../../../src/darwin/sdk-farm/link.h" $"($repo)/vendor/src/p1/escape.h"
