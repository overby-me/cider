...
#
#   scripts/checks/buck-upstream-names-check.nu
#   scripts/checks/buck-upstream-names-check.nu --refresh
#   scripts/checks/buck-upstream-names-check.nu --ours-root <dir>   # audit a tree that is not ours
#
# PORTED FROM PYTHON (#98). BYTE IDENTICAL on the real tree and on a planted control. --ours-root
# is what makes the control possible without editing src/linux/ or src/darwin/: point it at a directory
# holding one file that spells a name the Cider way and the check must name it.

const OURS = ["src" "src/linux" "src/darwin"]
