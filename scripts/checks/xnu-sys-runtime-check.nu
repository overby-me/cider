...
#
# Usage:
#   scripts/checks/xnu-sys-runtime-check.nu
#   scripts/checks/xnu-sys-runtime-check.nu --seconds 120
...
def run_one [target: string, verdict: string, covers: string, seconds: int] {
    ...
    let built = (do -i {
        ^buck2 build $"//src/linux/server:($target)" --show-output
    } | complete)
...
def link_check [] {
    ...
    let built = (do -i { ^buck2 build //src/linux/server:ciderd } | complete)
    if $built.exit_code != 0 {
...
# scripts/gen/gen-xnu-sys-traps.nu and CHECKED IN, so they can go stale if that table moves. The C
...
def traps_check [] {
    ...
    let r = (do -i { ^nu scripts/gen/gen-xnu-sys-traps.nu --check } | complete)
    if $r.exit_code != 0 {
