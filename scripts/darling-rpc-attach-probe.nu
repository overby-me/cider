#!/usr/bin/env nu
# Measure darlingserver overhead by attaching strace to the DAEMON itself (which lives outside
# the emulated process's user namespace, so its RPC-handling syscalls are actually visible --
# strace-ing `darling shell` only sees the launcher). Spawns N short-lived macho processes and
...
# darlingserver syscalls (ucred filled per message), fixed in
# patches/darlingserver/0002-cache-darlingserver-own-credentials.
#
# Converted from bash (task #40) and verified against it with darling, strace, pgrep and pkill
...
#     N=200 scripts/darling-rpc-attach-probe.nu
...
def main [
    ...
    --out: string = "/tmp/darling-rpc-probe"
] {
    let darling = ($env | get -o DARLING | default "darling")
    ...
    for p in [darlingserver mldr] { do -i { ^pkill -9 -x $p } }
    sleep 2sec
    ...
    let job = (job spawn { with-env {DPREFIX: $prefix} { do -i { ^$darling shell sh -c $loop_sh out+err> $loop_out } } })
    ...
    while ($ds | is-empty) and $c < 2000000 {
        $ds = (do -i { ^pgrep -x darlingserver | lines | first } | default "")
        $c = $c + 1
    }
    print $"darlingserver pid=($ds)"
    ...
    print $"=== darlingserver syscall histogram over ($n) spawns ==="
