...
xnu-sys inherits from parent scopes. Those were read off the configured
...
Usage:
  scripts/gen-xnu-sys-buck.py            # write the BUCK file
  scripts/gen-xnu-sys-buck.py --stdout   # print it instead
"""
...
FLAGS_BZL = os.path.join(REPO, "buck/generated/xnu_sys_flags.bzl")

# xnu-sys glue that has been ported to Rust (#71) and is no longer compiled into
# libciderd_xnu_sys.a. The Rust replacement lives in linux/server/src/ and exports
...
# xnu-sys's include_directories(), in order. Each becomes a cc_header_root.
...
def parse_library_sources(text: str) -> list[str]:
    m = re.search(r"add_library\(ciderd_xnu_sys STATIC\s*\n(.*?)\n\)", text, re.S)
    if not m:
        sys.exit("could not find add_library(ciderd_xnu_sys STATIC ...)")
    srcs = []
    for raw in m.group(1).split("\n"):
        line = raw.strip()
        if not line or line.startswith("#"):
...
def main(argv: list[str]) -> int:
    ...
    # be compiled anyway. These export macro-only xnu-sys operations as real symbols, because
    ...
    w("")
    w("# GENERATED by scripts/gen-xnu-sys-buck.py from CMakeLists.txt.")
    w("# Regenerate after an upstream bump; do not hand-edit.")
    w("#")
    w("# xnu-sys is the kernel-emulation glue that compiles the vendored XNU")
    w("# (osfmk/bsd) into libciderd_xnu_sys.a, which the Rust daemon links")
    w("# via XNU_SYS_LIB. Host tier: native ELF, no cross toolchain, no Mach-O.")
    w("")
    ...
    w('load("//buck/generated:xnu_sys_flags.bzl", "XNU_SYS_DEFINES", "XNU_SYS_FLAGS")')
    w("")
    ...
    w("# One target carrying xnu-sys's whole compile environment: the defines,")
    w("# the warning flags, and every include root, in the reference build's order.")
    w("# Everything that compiles xnu-sys code depends on just this.")
    w("cc_header_root(")
    w('    name = "dt_env",')
    w("    exported_flags = XNU_SYS_DEFINES + XNU_SYS_FLAGS,")
    w("    deps = [")
    ...
    w("")
    w("cc_static_lib(")
    w('    name = "ciderd_xnu_sys",')
    w('    lib_name = "ciderd_xnu_sys",')
    w("    objs = [")
    w('        ":dt_objects",')
    w('        ":dt_mig_objects",')
    ...
    bzl = ["# GENERATED by scripts/gen-xnu-sys-buck.py from CMakeLists.txt.",
           ...
           ""]
    bzl.append("XNU_SYS_DEFINES = [")
    bzl += [f'    "{d}",' for d in defines]
    bzl.append("]")
    bzl.append("")
    bzl.append("XNU_SYS_FLAGS = [")
