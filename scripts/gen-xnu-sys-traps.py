...
Usage:
  scripts/gen-xnu-sys-traps.py             # write linux/server/src/xnu/traps_generated.rs
  scripts/gen-xnu-sys-traps.py --check     # fail if the committed file is out of date
"""
...
def parse(header):
    ...
    m = re.search(r"#define DSERVER_XNU_SYS_DEFS \\\n(.*?)\n\n", text, re.S)
    if not m:
        sys.exit("DSERVER_XNU_SYS_DEFS not found in the header; did the generator change?")
    body = m.group(1).replace("\\\n", "\n")

    out = []
    for fn in re.findall(r"\tint xnu_sys_(\w+)\((.*?)\)\s*\{\s*"
                         r"struct (\w+) args = \{(.*?)\};\s*"
        ...
        for p in [x.strip() for x in raw_params.split(",") if x.strip()]:
            ...
            if not pm:
                sys.exit(f"cannot parse parameter {p!r} of xnu_sys_{name}")
            ctype, star, pname = pm.group(1).strip(), pm.group(2), pm.group(3)
            if ctype not in CTYPE:
                sys.exit(f"unknown C type {ctype!r} in xnu_sys_{name}; add it to CTYPE rather "
                         f"than guessing")
...
def render(wrappers):
    ...
    for name, params, struct, fields, trap in wrappers:
        ...
        lines.append(f"pub unsafe extern \"C\" fn xnu_sys_{name}({arglist}) "
                     f"-> ::std::os::raw::c_int {{")
...
def main():
    ...
    if args.check:
        if not os.path.exists(OUT):
            sys.exit(f"{OUT} does not exist; run scripts/gen-xnu-sys-traps.py")
        if open(OUT).read() != text:
            sys.exit(f"{OUT} is STALE: the RPC table has moved since it was generated.\n"
                     f"Re-run scripts/gen-xnu-sys-traps.py and commit the result.")
