...
#
# Usage:
#   scripts/gen/gen-xnu-sys-traps.nu             # write src/linux/server/src/xnu/traps_generated.rs
#   scripts/gen/gen-xnu-sys-traps.nu --check     # fail if the committed file is out of date
#
...
const OUT = "src/linux/server/src/xnu/traps_generated.rs"
...
def render [wrappers: list<any>] {
  mut lines = [
    ...
    "//! GENERATED by scripts/gen/gen-xnu-sys-traps.nu. Do not edit."
    "//!"
...
def main [--check] {
  ...
  if $check {
    if not ($OUT | path exists) {
      die $"($OUT) does not exist; run scripts/gen/gen-xnu-sys-traps.nu"
    }
    if (open --raw $OUT | decode utf-8) != $text {
      die ($"($OUT) is STALE: the RPC table has moved since it was generated.\n"
        + "Re-run scripts/gen/gen-xnu-sys-traps.nu and commit the result.")
    }
