...
#
# Usage:
#   scripts/gen/gen-xtrace-mig.nu --dry-run     # report only, changes nothing
#   scripts/gen/gen-xtrace-mig.nu               # WRITES: see the warning below
#
...
# src/darwin/launchd/BUCK, vendor/pins/ciderd/xnu-sys/BUCK and extra-deps.json, so the committed generated
...
def main [--dry-run] {
  ...
  for it in $extra_pairs {
    $extra = ($extra | upsert $it.lib [$"gen:($it.label)[xtrace]" "//src/darwin/xtrace:xtrace_headers"])
  }
