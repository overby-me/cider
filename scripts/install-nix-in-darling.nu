#!/usr/bin/env nu
# install-nix-in-darling.nu: automated Nix installer for Darling prefixes
#
...
#
# Converted from bash (task #40) and verified against it with curl, tar and darling stubbed on
...
#
# Usage:
#   ./scripts/install-nix-in-darling.nu [OPTIONS]
#
# Options:
...
def say [c: record, msg: string] { print $"($c.green)[darling-nix]($c.reset) ($msg)" }
def warn [c: record, msg: string] { print -e $"($c.yellow)[darling-nix] WARNING:($c.reset) ($msg)" }
def err_ [c: record, msg: string] { print -e $"($c.red)[darling-nix] ERROR:($c.reset) ($msg)" }
...
def dsh_bash [cmd: string] {
    ^darling shell bash -lc $cmd | complete
}

def main [
    --prefix: string = ""       # Darling prefix path (default: ~/.darling)
    ...
    if (which darling | is-empty) {
        err_ $c "darling is not installed or not in PATH.\n   Install it with: nix build .#darling"
        exit 1
    }

    if ($darling_prefix | path type) != "dir" {
        say $c $"Prefix ($darling_prefix) does not exist; initialising..."
        do -i { ^darling shell true } | ignore
        sleep 2sec
    }
    if ($darling_prefix | path type) != "dir" {
        err_ $c $"Failed to initialise Darling prefix at ($darling_prefix)"
        exit 1
    }
    ...
    say $c $"  Darling prefix: ($darling_prefix)"
    let sv = (dsh ["sw_vers" "-productVersion"])
    say $c $"  darling shell:  (if $sv.exit_code == 0 { $sv.stdout | str trim } else { 'unknown' })"
    ...
    let nix_conf = ([
        ...
        $"# Generated by install-nix-in-darling.nu on ($stamp)"
        ""
    ...
    mkdir $"($darling_prefix)/etc/nix"
    $"($nix_conf)\n" | save -f $"($darling_prefix)/etc/nix/nix.conf"
    ...
    # called nix-darling-install.XXXXXX, so head -1 picked the temp dir, whose install does not
    ...
    let darling_tmp = $"($darling_prefix)/private/tmp"
    mkdir $darling_tmp
    let guest_installer = $"($darling_tmp)/nix-installer"
    ...
    let run_rc = (try {
        ^darling shell env NIX_INSTALLER_NO_MODIFY_PROFILE=0 bash -ex /tmp/nix-installer/install --no-daemon out+err> $log_file
        0
    ...
    if $run_rc != 0 {
        ...
        err_ $c ""
        err_ $c "Debug tips:"
        err_ $c "  strace -f -p $(pidof darlingserver) -e trace=openat,stat 2>&1 | head -200"
        err_ $c "  DARLING_XTRACE=1 darling shell ..."
    ...
    let profile_d = $"($darling_prefix)/etc/profile.d"
    mkdir $profile_d
...
'
    $profile_body | save -f $"($profile_d)/nix-darling.sh"
    say $c "  Wrote /etc/profile.d/nix-darling.sh"
    ...
    if (not $no_channel) and (not $offline) {
        ...
        if (dsh_bash "nix-channel --add https://nixos.org/channels/nixpkgs-unstable nixpkgs").exit_code == 0 {
            ...
            if (dsh_bash "nix-channel --update").exit_code == 0 {
                ...
                warn $c "You can retry later with: darling shell nix-channel --update"
            }
        } else {
            warn $c "Failed to add channel \u{2014} Nix may not be on PATH yet."
            warn $c "Try: darling shell bash -lc 'nix-channel --add https://nixos.org/channels/nixpkgs-unstable nixpkgs'"
        }
    } else {
    ...
    say $c $"Prefix:      ($darling_prefix)"
    say $c $"Store:       ($darling_prefix)/nix/store"
    print ""
    say $c "Quick start:"
    say $c $"  ($c.blue)darling shell bash -lc 'nix --version'($c.reset)"
    say $c $"  ($c.blue)darling shell bash -lc 'nix eval --expr 1+1'($c.reset)"
    ...
    let qs = r##'darling shell bash -lc 'nix-build --expr "derivation { name = \"test\"; builder = \"/bin/bash\"; args = [\"-c\" \"echo ok > \\$out\"]; system = \"x86_64-darwin\"; }"''##
    say $c $"  ($c.blue)($qs)($c.reset)"
    print ""
    say $c "Or use the wrapper script:"
    say $c $"  ($c.blue)./scripts/darling-nix nix --version($c.reset)"
    say $c $"  ($c.blue)./scripts/darling-nix nix eval --expr '1 + 1'($c.reset)"
}
