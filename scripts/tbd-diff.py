...
Usage:
  scripts/tbd-diff.py --sdk <apple-sdk-path> \
      ...
      [--cider-root <cider prefix or dir of dylibs>] \
...
Typical:
  ...
  scripts/tbd-diff.py --sdk "$SDK" \
    --cider-root result/libexec/cider/usr/lib \
...
def main():
    ...
    ap.add_argument("--cider-root", help="dir of Darling's built dylibs")
    ...
    cider = None
    n_dylibs = 0
    if args.cider_root and os.path.isdir(args.cider_root):
        cider, n_dylibs = collect_cider(args.cider_root, args.arch)
    ...
    if cider is not None:
        lines.append(f"- Darling exported symbols ({n_dylibs} dylibs): **{len(cider)}**")
    ...
    if cider is not None:
        missing = {s: official[s] for s in official if s not in cider}
        report["cider_count"] = len(cider)
        report["missing_count"] = len(missing)
        lines.append(f"## Missing from Darling (official − cider): {len(missing)}\n")
        ...
        if demand is not None:
            ...
            lines.append(f"### Demanded work list (needed ∩ macOS14 − cider): "
                         f"**{len(worklist)}**\n")
        ...
        lines.append("_(no --cider-root given; supply-only run. Provide the "
