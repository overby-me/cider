...
# suitable for pasting into PLAN.md.
#
# Usage:
...
def main [
    ...
    if ($syscalls | is-not-empty) {
        ...
        for line in ($ordered | str replace -r '\n$' '' | split row "\n") {
            ...
            if not ($num in $seen) {
                ...
                $rep = ($rep | append $"- **Syscall ($num)** \(`($name)`\): Add to syscall triage table in `PLAN.md`")
            }
        }
        $rep = ($rep | append "")
    }
    ...
    $rep = ($rep | append [
        "### Next Steps"
        ""
        "1. Add any new syscalls to `PLAN.md`"
    ...
    $rep = ($rep | append [
        ""
        "---"
        "*Generated by `scripts/triage-syscalls.nu` \u{2014} see [PLAN.md](../PLAN.md)*"
    ])
