...
_vprocmgr_getsocket(name_t sockpath)
{
	/* SAY WHAT THE MACH CALL ANSWERED. Without this the only sign of failure is the ENOTCONN that
	 * liblaunch reports much later as "Socket is not connected", which says nothing about whether
	 * the message was refused at the send, timed out, or came back with an error from launchd. */
	kern_return_t kr = vproc_mig_getsocket(bootstrap_port, sockpath);

	syslog(LOG_ERR, "cider: _vprocmgr_getsocket: bootstrap_port=0x%x kr=0x%x (%d) path=%s",
			(unsigned int) bootstrap_port, (unsigned int) kr, (int) kr,
			(kr == 0) ? sockpath : "(none)");
