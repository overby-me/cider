#ifndef _CIDER_WAYLAND_CARBONKEYS_H
#define _CIDER_WAYLAND_CARBONKEYS_H

/*
 * EVDEV KEYCODE TO CARBON VIRTUAL KEY CODE.
 *
 * Copied from the X11 backend CarbonKeys.h, which is part of the same tree and the same licence.
 * It is copied rather than included because the X11 backend is going away, and it is REUSABLE
 * WITHOUT CHANGE because an X11 keycode and a Wayland one are the same number: both are the evdev
 * code plus eight. The letter h is 43 in each.
 *
 * WHY IT MATTERS. An application does not read text out of a key event: it reads the VIRTUAL KEY
 * CODE and applies its own layout mapping. LibreOffice does that, so sending it the raw evdev
 * code means it looks up whatever Carbon key happens to share that number, which is why every
 * keystroke arrived correctly and produced nothing in the document.
 */
...
static const uint8_t cider_evdev_to_carbon[256] = {
    [38] = kVK_ANSI_A,
    [39] = kVK_ANSI_S,
    [40] = kVK_ANSI_D,
    [41] = kVK_ANSI_F,
