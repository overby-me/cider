...
namespace {
	// darlingserver's own credentials (pid/uid/gid) are constant for the daemon's
	// lifetime: it never changes uid/gid after the startup root check, and it never
	// forks (managed processes are separate mldr-launched processes; workers are
	// threads that share this pid). Filling them from getpid()+getuid()+getgid() on
	// every Message previously cost three syscalls PER MESSAGE, which profiling
	// showed to be ~83% of darlingserver's entire syscall volume under a fork/exec
	// heavy build workload (~2100 of each per emulated process spawn). Cache once
	// via a thread-safe C++ magic static.
	const struct ucred& ourCachedCredentials() {
		static const struct ucred creds = []() {
			struct ucred c;
			c.pid = getpid();
			c.uid = getuid();
			c.gid = getgid();
	...
	_sendNotificationCallback(sendNotificationCallback)
{
	...
	memcpy(CMSG_DATA(_controlHeader), &ourCachedCredentials(), sizeof(struct ucred));
...
void DarlingServer::Message::_ensureCredentialsHeader() {
	...
	memcpy(CMSG_DATA(credHeader), &ourCachedCredentials(), sizeof(struct ucred));
};
