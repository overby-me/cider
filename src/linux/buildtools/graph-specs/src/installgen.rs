...
const EXTRA: &[(&str, &str)] = &[
    ("bin/cider", "//src/linux/launcher:cider"),
    ("bin/ciderd", "//src/linux/server:ciderd"),
    ("libexec/cider/usr/libexec/cider/mldr", "//src/darwin/loader:mldr"),
    (
...
const SOURCE_RENAMES: &[(&str, &str)] = &[
    (
        ...
        "src/darwin/shellspawn/me.overby.cider.shellspawn.plist",
    ),
    // etc/ WAS A TOP LEVEL DIRECTORY HOLDING ONE FILE. It became src/darwin/etc during the release
    // prep, because it is guest content and src/darwin/ is the guest side. Without this entry the
    ...
    ("etc/resolv.conf", "src/darwin/etc/resolv.conf"),
];
