...
pub fn project_candidates(tok: &str) -> Vec<String> {
    ...
    // `<flag>,<path>` in ONE argv token, so `-Wl,-alias_list,src/darwin/libm/Exports/x.alias` never
...
impl Fs {
    ...
    /// SOURCE ONLY TARGET IS INVISIBLE EVERYWHERE ELSE: //src/darwin/etc:resolv.conf appears in neither the
...
/// Where src/linux/buildtools/graph-specs/src/dump.rs parked a staged artifact. Same rule as the dump:
