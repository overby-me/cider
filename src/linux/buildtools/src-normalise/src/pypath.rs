...
/// os.path.abspath. The cwd is passed in rather than read here: the process never chdirs, so
/// reading it once is the same answer python gets on every call, and an explicit argument keeps
/// this module free of process state.
...
mod tests {
    ...
    fn matches_python() {
        ...
        assert_eq!(relpath("b/c", "b", "/root"), "c");
    }
}
