...
// INSTALL_PREFIX comes from $CIDER_INSTALL_PREFIX at build time (CMAKE_INSTALL_PREFIX
// in the C build); default is a placeholder since iteration passes $DSERVER_PATH.
use std::process::Command;

fn main() {
    let prefix =
        std::env::var("CIDER_INSTALL_PREFIX").unwrap_or_else(|_| "/usr/local".to_string());
    println!("cargo:rustc-env=CIDER_INSTALL_PREFIX={prefix}");
    println!("cargo:rerun-if-env-changed=CIDER_INSTALL_PREFIX");
    ...
    println!("cargo:rustc-env=CIDER_GIT_BRANCH={branch}");
    println!("cargo:rustc-env=CIDER_GIT_COMMIT={commit}");
}
