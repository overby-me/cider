...
//! xnu_sys_thread_sigexc_* calls in call.cpp's InterruptEnter/InterruptExit + thread.cpp.
//! See docs/changelog.md (bucket B.4).

use crate::bindings::xnu_sys_thread_t;
...
//
// What each one does:
//   xnu_sys_thread_dying          tears down a dead guest thread's Mach state (ports, rights,
...
//   xnu_sys_thread_sigexc_*       the signal-via-Mach-exception bracket, see the wrappers below.
...
/// up and no stale message is routed to it. Mirrors C++'s xnu_sys_thread_dying on checkout.
pub unsafe fn dying(thread: *mut xnu_sys_thread_t) {
...
pub unsafe fn load_state_from_user(thread: *mut xnu_sys_thread_t, thread_state: u64, float_state: u64) -> i32 {
    ...
    // written down.
    xnu_sys_thread_load_state_from_user(thread, thread_state as usize, float_state as usize)
}
/// Save the (possibly signal-modified) xnu_sys thread state back to the guest's addresses.
pub unsafe fn save_state_to_user(thread: *mut xnu_sys_thread_t, thread_state: u64, float_state: u64) -> i32 {
    xnu_sys_thread_save_state_to_user(thread, thread_state as usize, float_state as usize)
}
...
pub unsafe fn process_signal(thread: *mut xnu_sys_thread_t, bsd_signal: i32, linux_signal: i32, code: i32, signal_address: u64) {
    // signal_address is the third of the three that had been disagreeing: u64 on the wire,
    // usize at the xnu-sys entry point.
    xnu_sys_thread_process_signal(thread, bsd_signal, linux_signal, code, signal_address as usize);
}
/// Block while the thread is user-suspended (a debugger stopped it); returns immediately
/// if it is not suspended.
pub unsafe fn wait_while_user_suspended(thread: *mut xnu_sys_thread_t) {
    xnu_sys_thread_wait_while_user_suspended(thread);
}
...
pub unsafe fn sigexc_enter(thread: *mut xnu_sys_thread_t) {
    xnu_sys_thread_sigexc_enter(thread);
}
...
pub unsafe fn sigexc_enter2(thread: *mut xnu_sys_thread_t) {
    xnu_sys_thread_sigexc_enter2(thread);
}
...
pub unsafe fn sigexc_exit(thread: *mut xnu_sys_thread_t) {
    xnu_sys_thread_sigexc_exit(thread);
}
