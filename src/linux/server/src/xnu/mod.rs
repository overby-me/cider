...
// NAMED xnu_sys_kqchan because src/linux/server/src/kqchan.rs already exists and is the DAEMON side,
...
// xnu_sys_thread because src/linux/server/src/thread.rs is the daemon side.
pub mod thread;

// xnu-sys/src/task.c: task creation and teardown, task_info and the task_for_pid family
// (#71, fourteenth file). NAMED xnu_sys_task because src/linux/server/src/task.rs is the daemon side.
...
// NAMED xnu_sys_psynch because src/linux/server/src/psynch.rs already exists and is the DAEMON side,
...
// DSERVER_XNU_SYS_DEFS generates, emitted by scripts/gen/gen-xnu-sys-traps.nu.
pub mod traps;
