/* bindgen entry point for ciderd.
 *
 ...
 * owning_task->xnu_task, so Rust needs the offset of xnu_task inside struct xnu_sys_task,
 ...
 * (the RPC wrapper generator, via xnu-sys.h's DSERVER_XNU_SYS_DECLS) and the MIG output for
 * mach/task.h, where semaphore_create and semaphore_destroy are actually declared -- XNU's
 * kern/sync_sema.h declares only semaphore_destroy_all. src/linux/server/BUCK wires all of them
 ...
 * pre-existing 62 items come out byte for byte identical apart from xnu_sys_semaphore itself,
 ...
 *
 * task.h is listed even though semaphore.h already names xnu_sys_task_t, because it only
 * FORWARD-DECLARES it: without this line xnu_sys_task binds as an opaque [u8; 0] and the
 * xnu_task offset -- the one thing this half exists to provide -- is not there at all.
 */
#include <ciderd/xnu-sys/semaphore.h>
#include <ciderd/xnu-sys/task.h>
/* condvar.c walks back from a queue link to the containing xnu_sys_thread, so the port needs
...
#include <ciderd/xnu-sys/psynch.h>
...
#include <ciderd/xnu-sys/kqchan.h>
#include <sys/event.h>
...
#include <mach/kmod.h>
/* stubs.c takes XNU_SYS_FATAL_STUBS from the command line if it is set and 0 otherwise, and the
 * port has to make the same choice, so the same #ifndef decides it here. */
#ifndef XNU_SYS_FATAL_STUBS
	#define XNU_SYS_FATAL_STUBS 0
#endif
 ...
 */
struct xnu_sys_rs_task_id_token {
 ...
 */
enum xnu_sys_rs_host_consts {
	XNU_SYS_RS_HOST_BASIC_INFO_COUNT = HOST_BASIC_INFO_COUNT,
	XNU_SYS_RS_HOST_BASIC_INFO_OLD_COUNT = HOST_BASIC_INFO_OLD_COUNT,
	XNU_SYS_RS_HOST_PRIORITY_INFO_COUNT = HOST_PRIORITY_INFO_COUNT,
	XNU_SYS_RS_HOST_PREFERRED_USER_ARCH_COUNT = HOST_PREFERRED_USER_ARCH_COUNT,
	XNU_SYS_RS_HOST_VM_INFO_REV0_COUNT = HOST_VM_INFO_REV0_COUNT,
	XNU_SYS_RS_HOST_VM_INFO64_COUNT = HOST_VM_INFO64_COUNT,
	XNU_SYS_RS_CPU_TYPE_X86 = CPU_TYPE_X86,
	XNU_SYS_RS_CPU_SUBTYPE_X86_64_ALL = CPU_SUBTYPE_X86_64_ALL,
	XNU_SYS_RS_CPU_THREADTYPE_NONE = CPU_THREADTYPE_NONE,
	/* processor.c, same sizeof-expression shape as the host counts above. */
	XNU_SYS_RS_PROCESSOR_BASIC_INFO_COUNT = PROCESSOR_BASIC_INFO_COUNT,
	XNU_SYS_RS_PROCESSOR_CPU_LOAD_INFO_COUNT = PROCESSOR_CPU_LOAD_INFO_COUNT,
	XNU_SYS_RS_PROCESSOR_SET_LOAD_INFO_COUNT = PROCESSOR_SET_LOAD_INFO_COUNT,
	XNU_SYS_RS_MAX_SCHED_CPUS = MAX_SCHED_CPUS,
	XNU_SYS_RS_PROCESSOR_SET_BASIC_INFO_COUNT = PROCESSOR_SET_BASIC_INFO_COUNT,
	XNU_SYS_RS_POLICY_TIMESHARE_LIMIT_COUNT = POLICY_TIMESHARE_LIMIT_COUNT,
	XNU_SYS_RS_POLICY_FIFO_LIMIT_COUNT = POLICY_FIFO_LIMIT_COUNT,
	XNU_SYS_RS_POLICY_RR_LIMIT_COUNT = POLICY_RR_LIMIT_COUNT,
	XNU_SYS_RS_POLICY_TIMESHARE_BASE_COUNT = POLICY_TIMESHARE_BASE_COUNT,
	XNU_SYS_RS_POLICY_FIFO_BASE_COUNT = POLICY_FIFO_BASE_COUNT,
	XNU_SYS_RS_POLICY_RR_BASE_COUNT = POLICY_RR_BASE_COUNT,
	/* init.c: sizes for zone_create, and the one macro-valued size among them. */
	XNU_SYS_RS_SIZEOF_TASK_ID_TOKEN = sizeof(struct xnu_sys_rs_task_id_token),
	XNU_SYS_RS_IKM_SAVED_KMSG_SIZE = IKM_SAVED_KMSG_SIZE,
	/* kqchan.c: anonymous-enum values and macros bindgen does not surface by name. */
	XNU_SYS_RS_KN_VANISHED = KN_VANISHED,
	XNU_SYS_RS_THREAD_INTERRUPTIBLE = THREAD_INTERRUPTIBLE,
	XNU_SYS_RS_THREAD_INTERRUPTED = THREAD_INTERRUPTED,
	/* psynch.c: more anonymous-enum wait results and the abortsafe level. */
	XNU_SYS_RS_THREAD_ABORTSAFE = THREAD_ABORTSAFE,
	XNU_SYS_RS_THREAD_AWAKENED = THREAD_AWAKENED,
	XNU_SYS_RS_THREAD_RESTART = THREAD_RESTART,
	XNU_SYS_RS_THREAD_TIMED_OUT = THREAD_TIMED_OUT,
	XNU_SYS_RS_THREAD_UNINT = THREAD_UNINT,
	...
	XNU_SYS_RS_CONFIG_THREAD_MAX = CONFIG_THREAD_MAX,
	...
	XNU_SYS_RS_MACH_VM_MAX_ADDRESS = MACH_VM_MAX_ADDRESS,
	XNU_SYS_RS_VM_REGION_BASIC_INFO_COUNT = VM_REGION_BASIC_INFO_COUNT,
	XNU_SYS_RS_VM_REGION_BASIC_INFO_COUNT_64 = VM_REGION_BASIC_INFO_COUNT_64,
	XNU_SYS_RS_VM_REGION_SUBMAP_SHORT_INFO_COUNT_64 = VM_REGION_SUBMAP_SHORT_INFO_COUNT_64,
	...
	XNU_SYS_RS_VM_PROT_NONE = VM_PROT_NONE,
	XNU_SYS_RS_VM_PROT_READ = VM_PROT_READ,
	XNU_SYS_RS_VM_PROT_WRITE = VM_PROT_WRITE,
	XNU_SYS_RS_VM_PROT_EXECUTE = VM_PROT_EXECUTE,
	XNU_SYS_RS_VM_PROT_ALL = VM_PROT_ALL,
	XNU_SYS_RS_VM_SYNC_ASYNCHRONOUS = VM_SYNC_ASYNCHRONOUS,
	XNU_SYS_RS_VM_SYNC_SYNCHRONOUS = VM_SYNC_SYNCHRONOUS,
	XNU_SYS_RS_VM_SYNC_INVALIDATE = VM_SYNC_INVALIDATE,
	XNU_SYS_RS_VM_BEHAVIOR_DEFAULT = VM_BEHAVIOR_DEFAULT,
	/* task.c: the default role, an enum variant of a type nothing else needs. */
	XNU_SYS_RS_TASK_UNSPECIFIED = TASK_UNSPECIFIED,
	/* task.c: the info-flavor counts, every one a sizeof expression the C must evaluate. */
	XNU_SYS_RS_TASK_BASIC_INFO_32_COUNT = TASK_BASIC_INFO_32_COUNT,
	XNU_SYS_RS_TASK_BASIC_INFO_64_COUNT = TASK_BASIC_INFO_64_COUNT,
	XNU_SYS_RS_MACH_TASK_BASIC_INFO_COUNT = MACH_TASK_BASIC_INFO_COUNT,
	XNU_SYS_RS_TASK_THREAD_TIMES_INFO_COUNT = TASK_THREAD_TIMES_INFO_COUNT,
	XNU_SYS_RS_TASK_DYLD_INFO_COUNT = TASK_DYLD_INFO_COUNT,
	XNU_SYS_RS_TASK_AUDIT_TOKEN_COUNT = TASK_AUDIT_TOKEN_COUNT,
	XNU_SYS_RS_TASK_VM_INFO_REV0_COUNT = TASK_VM_INFO_REV0_COUNT,
	XNU_SYS_RS_TASK_VM_INFO_REV1_COUNT = TASK_VM_INFO_REV1_COUNT,
	XNU_SYS_RS_TASK_VM_INFO_REV2_COUNT = TASK_VM_INFO_REV2_COUNT,
	XNU_SYS_RS_TASK_VM_INFO_REV3_COUNT = TASK_VM_INFO_REV3_COUNT,
	XNU_SYS_RS_TASK_VM_INFO_REV4_COUNT = TASK_VM_INFO_REV4_COUNT,
	XNU_SYS_RS_TASK_VM_INFO_REV5_COUNT = TASK_VM_INFO_REV5_COUNT,
	XNU_SYS_RS_TASK_FLAGS_INFO_COUNT = TASK_FLAGS_INFO_COUNT,
	...
	XNU_SYS_RS_XNU_SYS_FATAL_STUBS = XNU_SYS_FATAL_STUBS,
	 ...
	 * These are the C compiler answers, asserted against Rust in src/linux/server/src/layout.rs. */
	XNU_SYS_RS_SIZEOF_XNU_TASK = sizeof(struct task),
	XNU_SYS_RS_SIZEOF_XNU_THREAD = sizeof(struct thread),
	XNU_SYS_RS_SIZEOF_XNU_SYS_TASK = sizeof(struct xnu_sys_task),
	XNU_SYS_RS_SIZEOF_XNU_SYS_THREAD = sizeof(struct xnu_sys_thread),
	XNU_SYS_RS_OFFSETOF_XNU_SYS_TASK_XNU_TASK = __builtin_offsetof(struct xnu_sys_task, xnu_task),
	XNU_SYS_RS_OFFSETOF_XNU_SYS_THREAD_XNU_THREAD = __builtin_offsetof(struct xnu_sys_thread, xnu_thread),
	...
	XNU_SYS_RS_X86_THREAD_STATE64_COUNT = x86_THREAD_STATE64_COUNT,
	XNU_SYS_RS_X86_THREAD_FULL_STATE64 = x86_THREAD_FULL_STATE64,
	XNU_SYS_RS_X86_THREAD_FULL_STATE64_COUNT = x86_THREAD_FULL_STATE64_COUNT,
	XNU_SYS_RS_X86_THREAD_STATE = x86_THREAD_STATE,
	XNU_SYS_RS_X86_THREAD_STATE_COUNT = x86_THREAD_STATE_COUNT,
	XNU_SYS_RS_X86_FLOAT_STATE32 = x86_FLOAT_STATE32,
	XNU_SYS_RS_X86_FLOAT_STATE32_COUNT = x86_FLOAT_STATE32_COUNT,
	XNU_SYS_RS_X86_FLOAT_STATE64 = x86_FLOAT_STATE64,
	XNU_SYS_RS_X86_FLOAT_STATE64_COUNT = x86_FLOAT_STATE64_COUNT,
	XNU_SYS_RS_X86_FLOAT_STATE = x86_FLOAT_STATE,
	XNU_SYS_RS_X86_FLOAT_STATE_COUNT = x86_FLOAT_STATE_COUNT,
	XNU_SYS_RS_X86_EXCEPTION_STATE32 = x86_EXCEPTION_STATE32,
	XNU_SYS_RS_X86_EXCEPTION_STATE32_COUNT = x86_EXCEPTION_STATE32_COUNT,
	XNU_SYS_RS_X86_EXCEPTION_STATE64 = x86_EXCEPTION_STATE64,
	XNU_SYS_RS_X86_EXCEPTION_STATE64_COUNT = x86_EXCEPTION_STATE64_COUNT,
	XNU_SYS_RS_X86_EXCEPTION_STATE = x86_EXCEPTION_STATE,
	XNU_SYS_RS_X86_EXCEPTION_STATE_COUNT = x86_EXCEPTION_STATE_COUNT,
	XNU_SYS_RS_X86_DEBUG_STATE32 = x86_DEBUG_STATE32,
	XNU_SYS_RS_X86_DEBUG_STATE32_COUNT = x86_DEBUG_STATE32_COUNT,
	XNU_SYS_RS_X86_DEBUG_STATE64 = x86_DEBUG_STATE64,
	XNU_SYS_RS_X86_DEBUG_STATE64_COUNT = x86_DEBUG_STATE64_COUNT,
	XNU_SYS_RS_X86_DEBUG_STATE = x86_DEBUG_STATE,
	XNU_SYS_RS_X86_DEBUG_STATE_COUNT = x86_DEBUG_STATE_COUNT,
	XNU_SYS_RS_X86_AVX_STATE32 = x86_AVX_STATE32,
	XNU_SYS_RS_X86_AVX_STATE32_COUNT = x86_AVX_STATE32_COUNT,
	XNU_SYS_RS_X86_AVX_STATE64 = x86_AVX_STATE64,
	XNU_SYS_RS_X86_AVX_STATE64_COUNT = x86_AVX_STATE64_COUNT,
	XNU_SYS_RS_X86_AVX_STATE = x86_AVX_STATE,
	XNU_SYS_RS_X86_AVX_STATE_COUNT = x86_AVX_STATE_COUNT,
	XNU_SYS_RS_X86_AVX512_STATE32 = x86_AVX512_STATE32,
	XNU_SYS_RS_X86_AVX512_STATE32_COUNT = x86_AVX512_STATE32_COUNT,
	XNU_SYS_RS_X86_AVX512_STATE64 = x86_AVX512_STATE64,
	XNU_SYS_RS_X86_AVX512_STATE64_COUNT = x86_AVX512_STATE64_COUNT,
	XNU_SYS_RS_X86_AVX512_STATE = x86_AVX512_STATE,
