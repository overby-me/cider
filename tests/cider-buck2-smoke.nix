...
#
# tests/cider-smoke.nix is the endpoint, and it cannot pass yet: it exercises uname,
...
#
# Usage:
#   nix build .#checks.x86_64-linux.cider-buck2-smoke -L
{
  pkgs,
  cider,
  ...
  ...
  nixos-lib.runTest {
    ...
    testScript = ''
      ...
      with machine.nested("the launcher is installed"):
          machine.succeed("command -v cider-buck2")
      ...
      with machine.nested("the container boots and bash runs"):
          ...
          if "BUCK2_BASH_OK" not in out:
              ...
              machine.execute("tail -40 /tmp/dp/ciderd.log > /tmp/ds.log 2>&1")
              ...
              st_b, procs = machine.execute(
                  ...
                  "grep -E 'cider|mldr|shellspawn' | grep -v grep"
              )
              ...
              # reason the earlier "dtype for fd 2 -> /ciderd.log" line was written up
              ...
              st_d, fdinfo = machine.execute(
                  "for p in $(pgrep -x ciderd; pgrep -x mldr; pgrep -x cider-buck2); do "
                  ...
                  "echo \"ciderd: $(pgrep -x -c ciderd 2>/dev/null || echo 0)\""
              )
              raise Exception(
                  f"status={status}\noutput={out!r}\n"
                  f"--- daemon log ---\n{dslog}\n--- processes ---\n{procs}\n"
