...
#
#   1. The darling-builder systemd service starts and initialises the prefix
...
nixos-lib.runTest {
  ...
  nodes.machine =
    { config, pkgs, lib, ... }:
    {
      ...
      # in the prefix (which the full nix-in-darling test covers).
  ...
  testScript = ''
    ...
    with machine.nested("Stage 1: Darling builder service starts"):
        ...
        machine.wait_for_unit("darling-builder.service", timeout=300)
        machine.log("darling-builder.service is active")

        # Verify the prefix directory was created
        machine.succeed("test -d /var/lib/darling-builder")

    # ── Stage 2: SSH keys were generated ───────────────────────────────

    with machine.nested("Stage 2: SSH keys were generated"):
        machine.succeed("test -f /etc/nix/darling-builder-key")
        machine.succeed("test -f /etc/nix/darling-builder-key.pub")

        # Key should be ed25519
        result = machine.succeed("head -1 /etc/nix/darling-builder-key.pub")
    ...
    with machine.nested("Stage 9: Directory Services stubs work via SSH"):
        ...
        for tool in ["dseditgroup", "sysadminctl", "dscl"]:
            machine.succeed(
                ...
                f"-i /etc/nix/darling-builder-key "
                f"-p 2222 "
                f"root@127.0.0.1 "
    ...
    with machine.nested("Stage 12: Service restart resilience"):
        ...
        machine.succeed("systemctl restart darling-builder.service")
        machine.wait_for_unit("darling-builder.service", timeout=300)
