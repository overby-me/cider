...
nixos-lib.runTest {
  ...
  testScript = ''
    ...
    with machine.nested("Stage 1: Darling binary exists and prefix initialises"):
        machine.succeed("command -v darling")

        # First invocation creates the prefix — may take a while
        machine.succeed("darling shell true", timeout=120)

    # ── Stage 2: Basic shell functionality ─────────────────────────────

    with machine.nested("Stage 2: Basic shell functionality"):
        result = machine.succeed("darling shell echo 'hello from darling'")
        assert "hello from darling" in result, f"echo failed: {result}"

        # Environment variables pass through
        result = machine.succeed("darling shell /bin/bash -c 'echo $HOME'")
        assert result.strip(), f"HOME was empty: {result}"

        # Exit codes propagate
        machine.succeed("darling shell /bin/bash -c 'exit 0'")
        machine.fail("darling shell /bin/bash -c 'exit 1'")

    # ── Stage 3: macOS identity ────────────────────────────────────────

    with machine.nested("Stage 3: macOS identity"):
        result = machine.succeed("darling shell uname -s")
        assert "Darwin" in result, f"Expected 'Darwin', got: {result}"

        result = machine.succeed("darling shell uname -m")
    ...
    with machine.nested("Stage 4: Filesystem basics"):
        ...
        machine.succeed(
            "darling shell /bin/bash -c 'echo test-content > /tmp/smoke-test.txt'"
        )
        result = machine.succeed("darling shell cat /tmp/smoke-test.txt")
        assert "test-content" in result, f"File content mismatch: {result}"

        # Can create directories
        machine.succeed("darling shell mkdir -p /tmp/smoke-dir/sub")
        machine.succeed("darling shell test -d /tmp/smoke-dir/sub")

        # Symlinks work
        machine.succeed(
            "darling shell ln -sf /tmp/smoke-test.txt /tmp/smoke-link"
        )
        result = machine.succeed("darling shell cat /tmp/smoke-link")
        assert "test-content" in result, f"Symlink read failed: {result}"

        # Can remove files
        machine.succeed("darling shell rm /tmp/smoke-test.txt /tmp/smoke-link")
        machine.succeed("darling shell rm -rf /tmp/smoke-dir")
    ...
    with machine.nested("Stage 5: sandbox-exec stub"):
        ...
        result = machine.succeed(
            "darling shell /usr/bin/sandbox-exec -f /dev/null /bin/echo sandbox-ok"
        )
        assert "sandbox-ok" in result, f"sandbox-exec passthrough failed: {result}"

        # With -p (inline profile string)
        result = machine.succeed(
            "darling shell /usr/bin/sandbox-exec -p '(version 1)(allow default)' "
            "/bin/echo inline-ok"
        )
        ...
        result = machine.succeed(
            "darling shell /usr/bin/sandbox-exec "
            "-f /dev/null "
        ...
        machine.fail(
            "darling shell /usr/bin/sandbox-exec -f /dev/null /bin/bash -c 'exit 42'"
        )

    # ── Stage 6: diskutil stub ─────────────────────────────────────────

    with machine.nested("Stage 6: diskutil stub"):
        machine.succeed("darling shell test -x /usr/sbin/diskutil")

        # diskutil info /
        result = machine.succeed("darling shell /usr/sbin/diskutil info /")
        assert "APFS" in result or "apfs" in result, (
            f"Expected APFS in diskutil info output: {result}"
        )

        # diskutil info -plist /
        result = machine.succeed("darling shell /usr/sbin/diskutil info -plist /")
        ...
        # diskutil list
        result = machine.succeed("darling shell /usr/sbin/diskutil list")
    ...
    with machine.nested("Stage 7: Directory Services stubs"):
        for tool in ["dseditgroup", "sysadminctl", "dscl"]:
            machine.succeed(f"darling shell test -x /usr/sbin/{tool}")

        # Create a test group
        machine.succeed(
            "darling shell /usr/sbin/dseditgroup -o create -q -i 39999 smoketest"
        )
        result = machine.succeed("darling shell grep smoketest /etc/group")
        assert "39999" in result, f"Group GID mismatch: {result}"

        # Create a test user
        machine.succeed(
            "darling shell /usr/sbin/sysadminctl -addUser _smoketest "
            "-UID 399 -GID 39999 -home /var/empty -shell /usr/bin/false"
        )
        result = machine.succeed("darling shell grep _smoketest /etc/passwd")
        assert "399" in result, f"User UID mismatch: {result}"

        # Add user to group
        machine.succeed(
            "darling shell /usr/sbin/dseditgroup -o edit "
            "-a _smoketest -t user smoketest"
        )

        # Verify via dscl
        result = machine.succeed(
            "darling shell /usr/sbin/dscl . -read /Groups/smoketest GroupMembership"
        )
        assert "_smoketest" in result, f"Member not found via dscl: {result}"

        result = machine.succeed(
            "darling shell /usr/sbin/dscl . -read /Users/_smoketest UniqueID"
        )
        assert "399" in result, f"UID not found via dscl: {result}"

        # Checkmember
        machine.succeed(
            "darling shell /usr/sbin/dseditgroup -o checkmember "
            "-m _smoketest smoketest"
        )

        # Cleanup
        machine.succeed(
            "darling shell /usr/sbin/sysadminctl -deleteUser _smoketest"
        )
        machine.succeed(
            "darling shell /usr/sbin/dseditgroup -o delete smoketest"
        )
