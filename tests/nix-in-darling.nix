...
#
# Test stages:
#   1. Darling boots and `darling shell` is functional
...
nixos-lib.runTest {
  ...
  testScript = ''
    ...
    with machine.nested("Stage 1: Darling boots and shell is functional"):
        # Verify darling binary is present
    ...
    with machine.nested("Stage 3: Directory Services stubs are present"):
        ...
        machine.succeed(
            "darling shell /usr/sbin/dseditgroup -o create -q -i 30000 nixbld"
        )
        result = machine.succeed("darling shell grep nixbld /etc/group")
        assert "30000" in result, f"Expected GID 30000 in group entry, got: {result}"

        machine.succeed(
            "darling shell /usr/sbin/sysadminctl -addUser _nixbld1 "
            "-UID 300 -GID 30000 -home /var/empty -shell /usr/bin/false"
        )
        result = machine.succeed("darling shell grep _nixbld1 /etc/passwd")
        assert "300" in result, f"Expected UID 300 in passwd entry, got: {result}"

        # Add user to group
        machine.succeed(
            "darling shell /usr/sbin/dseditgroup -o edit -a _nixbld1 -t user nixbld"
        )

        # Verify with dscl
        result = machine.succeed(
            "darling shell /usr/sbin/dscl . -read /Groups/nixbld GroupMembership"
        )
    ...
    with machine.nested("Stage 4: Pre-configure and install Nix"):
        ...
        machine.succeed("mkdir -p ~/.darling/etc/nix")
        machine.succeed(
            "cat > ~/.darling/etc/nix/nix.conf << 'CONF'\n"
            "build-users-group =\n"
            "sandbox = false\n"
        ...
        machine.succeed("darling shell mkdir -p /nix")
        ...
        machine.succeed("mkdir -p ~/.darling/private/tmp/nix-installer")
        machine.succeed(
            f"cp -a {installer_dir}/* ~/.darling/private/tmp/nix-installer/"
        )
        machine.succeed("chmod +x ~/.darling/private/tmp/nix-installer/install")

        # Copy the nix-run helper into the prefix
        machine.succeed(
            "cp ${nixRunHelper} ~/.darling/private/tmp/nix-run && "
            "chmod +x ~/.darling/private/tmp/nix-run"
        )

        # Run the patched installer inside Darling
        machine.succeed(
            "darling shell env NIX_INSTALLER_NO_MODIFY_PROFILE=0 "
    ...
    with machine.nested("Stage 5: Core Nix commands work"):
        ...
        # nix --version
        result = machine.succeed(
            f"darling shell {nix_run} nix --version"
        )
        ...
        # nix-env --version
        result = machine.succeed(
            f"darling shell {nix_run} nix-env --version"
        )
        ...
        result = machine.succeed(
            f"darling shell {nix_run} nix-instantiate --eval -E '1 + 1'"
        )
        assert "2" in result, f"Expected '2', got: {result}"

        # nix eval (flake-style)
        result = machine.succeed(
            f"darling shell {nix_run} nix eval --expr '1 + 1'"
        )
        assert "2" in result, f"Expected '2', got: {result}"

        # Verify store database is accessible
        machine.succeed(
            f"darling shell {nix_run} nix-store --verify --no-build",
            timeout=120,
        )
    ...
    with machine.nested("Stage 6: builtins.currentSystem reports x86_64-darwin"):
        ...
        result = machine.succeed(
            f"darling shell {nix_run} nix eval --raw --expr builtins.currentSystem"
        )
    ...
    with machine.nested("Stage 7: Trivial derivation builds"):
        ...
        machine.succeed(
            "cp ${trivialDrvFile} ~/.darling/private/tmp/trivial-drv.nix"
        )
        machine.succeed(
            "cp ${multistepDrvFile} ~/.darling/private/tmp/multistep-drv.nix"
        )

        # Level 1: minimal derivation — echo to $out
        out_path = machine.succeed(
            f"darling shell {nix_run} nix-build --no-out-link /tmp/trivial-drv.nix",
            timeout=300,
        ).strip()
