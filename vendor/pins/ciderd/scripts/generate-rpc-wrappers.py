...
calls = [
	...
	#       we only use our RPC for ciderd calls.
	#
	...
	('mach_port_allocate', [
		...
		# but due to the way ciderd handles Mach IPC calls,
...
license_header = """\
// This file has been auto-generated by generate-rpc-wrappers.py for use with ciderd
...
# ---- Rust RPC wire codec (Stage 1 of the host-side Rust rewrite) ----
...
# The C outputs above are untouched. See docs/changelog.md (Stage 1).
if len(sys.argv) > 5:
	RUST_WIRE_MAP = {
		'bool': 'bool',
	...
	RUST_KEYWORDS = {
		...
		'use','where','while','async','await','box','abstract','become','final','macro',
		'override','priv','typeof','unsized','virtual','yield',
	}

	def rust_wire_type(param):
		# mirror parse_type(param, is_public=False)
		t = param[1].strip()
		if t == '@fd':
			t = 'int32_t'
		elif len(param) > 2:
			t = param[2].strip()
		return RUST_WIRE_MAP[t]

	def rust_field_name(name):
		return ('r#' + name) if name in RUST_KEYWORDS else name

	def emit_body_struct(rf, struct_name, params):
		rf.write("#[repr(C)]\n#[derive(Clone, Copy, Debug)]\npub struct " + struct_name + " {\n")
		for p in params:
			rf.write("\tpub " + rust_field_name(p[0]) + ": " + rust_wire_type(p) + ",\n")
		rf.write("}\n")

	rf = open(sys.argv[5], "w")
	rf.write("// This file has been auto-generated by generate-rpc-wrappers.py -- DO NOT EDIT.\n")
	rf.write("// Byte-identical Rust mirror of the ciderd RPC wire messages, emitted\n")
	rf.write("// from the same `calls` source of truth as the C headers. x86_64 layout.\n")
	rf.write("#![allow(non_camel_case_types, non_snake_case, dead_code)]\n\n")

	rf.write("#[repr(C)]\n#[derive(Clone, Copy, Debug)]\npub struct DserverRpcCallhdr {\n")
	rf.write("\tpub number: u32,\n\tpub pid: i32,\n\tpub tid: i32,\n\tpub architecture: u32,\n}\n\n")
	rf.write("#[repr(C)]\n#[derive(Clone, Copy, Debug)]\npub struct DserverRpcReplyhdr {\n")
	rf.write("\tpub number: u32,\n\tpub code: i32,\n}\n\n")

	rf.write("/// RPC call numbers (matches enum dserver_callnum in the C header).\n")
	rf.write("pub mod callnum {\n")
	rf.write("\tpub const UNMANAGED_FLAG: u32 = 0x80000000;\n")
	rf.write("\tpub const S2C: u32 = 0x52cca11;\n")
	rf.write("\tpub const PUSH_REPLY: u32 = 0xbadca11;\n")
	rf.write("\tpub const INVALID: u32 = 0;\n")
	idx = 1
	for call in calls:
		flags = call[3] if len(call) >= 4 else 0
		val = ("UNMANAGED_FLAG | " + str(idx)) if (flags & UNMANAGED_CALL) != 0 else str(idx)
		rf.write("\tpub const " + call[0].upper() + ": u32 = " + val + ";\n")
		idx += 1
	rf.write("}\n\n")
	...
	for call in calls:
		...
		if len(call_params) > 0:
			emit_body_struct(rf, "Call" + camel, call_params)
		rf.write("#[repr(C)]\n#[derive(Clone, Copy, Debug)]\npub struct RpcCall" + camel + " {\n")
		rf.write("\tpub header: DserverRpcCallhdr,\n")
		if len(call_params) > 0:
			rf.write("\tpub body: Call" + camel + ",\n")
		rf.write("}\n")

		if len(reply_params) > 0:
			emit_body_struct(rf, "Reply" + camel, reply_params)
		rf.write("#[repr(C)]\n#[derive(Clone, Copy, Debug)]\npub struct RpcReply" + camel + " {\n")
		rf.write("\tpub header: DserverRpcReplyhdr,\n")
		if len(reply_params) > 0:
			rf.write("\tpub body: Reply" + camel + ",\n")
		rf.write("}\n\n")
	...
	rf.write("\tprintln!(\"replyhdr {} {}\", size_of::<DserverRpcReplyhdr>(), align_of::<DserverRpcReplyhdr>());\n")
	for call in calls:
...
# against the real generated public header. Diffing its output against print_sizes()
# proves the Rust wire structs are byte-identical to the C the clients use.
if len(sys.argv) > 6:
	cf = open(sys.argv[6], "w")
	cf.write("/* Auto-generated by generate-rpc-wrappers.py -- byte-parity probe. */\n")
	cf.write("#include \"" + os.path.abspath(sys.argv[1]) + "\"\n")
	cf.write("#include <stdio.h>\n#include <stddef.h>\n\n")
	cf.write("int main(void) {\n")
	cf.write("\tprintf(\"callhdr %zu %zu\\n\", sizeof(dserver_rpc_callhdr_t), _Alignof(dserver_rpc_callhdr_t));\n")
	cf.write("\tprintf(\"replyhdr %zu %zu\\n\", sizeof(dserver_rpc_replyhdr_t), _Alignof(dserver_rpc_replyhdr_t));\n")
	for call in calls:
		name = call[0]
		camel = to_camel_case(name)
		cf.write("\tprintf(\"RpcCall" + camel + " %zu %zu\\n\", sizeof(dserver_rpc_call_" + name + "_t), _Alignof(dserver_rpc_call_" + name + "_t));\n")
		cf.write("\tprintf(\"RpcReply" + camel + " %zu %zu\\n\", sizeof(dserver_rpc_reply_" + name + "_t), _Alignof(dserver_rpc_reply_" + name + "_t));\n")
	cf.write("\treturn 0;\n}\n")
	cf.close()
